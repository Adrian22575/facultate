...
| Feedback inline | `InlineFeedback` din `components/ui/status.js` | `components/ui/status.module.css` | `error`, `success`, `info`, `warning` | Pentru feedback accesibil asociat unei acțiuni sau unui formular. Eroarea are implicit `role="alert"`; celelalte tonuri folosesc `role="status"`. Migrarea inițială include feedback-ul `GoogleSignInButton`. |
...
| Tabele de date | `DataTable`, `DataTableCell` din `components/ui/data-table.js` | `components/ui/data-table.module.css` | responsive `scroll`/`cards`; celule `text`, `name`, `code`, `date`, `count`, `link`; lățimi `wide`, `xl`, `xxl` | Pentru comparație pe rânduri și coloane. Caption-ul este obligatoriu, headerele sunt generate cu `scope="col"`, iar strategia cards cere `data-label` pe fiecare celulă. Acțiunile, selecția și stările rămân responsabilitatea consumatorului. |
| Etichet? de sec?iune | `SectionLabel` din `components/ui/section-label.js` | `components/ui/section-label.module.css` | element semantic configurabil prin `as` | Pentru etichete scurte care preced un heading. Clasele contextuale se transmit prin `className`; componenta nu stabile?te structura sec?iunii. |
| Bar? de progres | `ProgressBar` din `components/ui/progress-bar.js` | `components/ui/progress-bar.module.css` | valoare limitat? automat la `0?100`; clase contextuale pentru track ?i fill | Pentru progres determinist. Consumatorul p?streaz? labelul accesibil ?i poate colora local fill-ul f?r? s? recreeze geometria track-ului. |
| Structură dialog | `DialogShell` din `components/ui/dialog-shell.js` | `components/ui/dialog-shell.module.css` | panel, header, close, body și actions | Pentru structura vizuală repetată a dialogurilor. Proprietarul păstrează portalul, focus trap-ul, Escape și restaurarea focusului. |

...
import optionStyles from "./linkedin-generation-options.module.css";
...
function OptionField({ label, options, value, onChange, disabled }) {
  ...
  return (
    <label className={moduleClassNames(optionStyles, "linkedin-option-field")}>
      <span>{label}</span>
...
export function LinkedInGenerationOptions({ value, onChange, disabled = false, compact = false }) {
  ...
  return (
    <div className={moduleClassNames(optionStyles, `linkedin-generation-options${compact ? " is-compact" : ""}`)}>
      <div className={moduleClassNames(optionStyles, "linkedin-generation-options-primary")}>
        ...
        {value.audienceKey === "custom" ? <label className={moduleClassNames(optionStyles, "linkedin-option-field is-custom")}><span>Audiența exactă</span><input value={value.customAudience || ""} maxLength={180} disabled={disabled} onChange={(event) => set("customAudience")(event.target.value)} placeholder="Ex.: directori de școli private din România" /><small>Descrie rolul, industria sau comunitatea vizată.</small></label> : null}
      </div>
      <details className={moduleClassNames(optionStyles, "linkedin-generation-advanced")}>

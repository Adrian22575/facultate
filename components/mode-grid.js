...
import styles from "./mode-grid.module.css";
...
function WelcomePremiumInlineCard({ returnTo }) {
  return (
    <article className={moduleClassNames([styles], "subject-welcome-premium")} aria-label="Bonus premium disponibil">
      <div className={moduleClassNames([styles], "subject-welcome-icon")} aria-hidden="true">
      ...
      <div className={moduleClassNames([styles], "subject-welcome-copy")}>
        <strong>Ai 24h Premium cadou</strong>
        <span>Activeaza si foloseste Interactiv, Studiaza si Test.</span>
      </div>
      <form action={activateWelcomePremiumAction} className={moduleClassNames([styles], "subject-welcome-form")}>
        <input type="hidden" name="returnTo" value={returnTo} />
        <button type="submit">Activeaza acum</button>
      </form>
      <span className={moduleClassNames([styles], "subject-welcome-note")}>Porneste doar dupa activare.</span>
    </article>
  );
}
...
export function ModeGrid({
  ...
  return (
    <section className={moduleClassNames([styles], "mode-grid-panel")}>
      {welcomeState === "activated" ? (
        <div className={moduleClassNames([styles], "subject-welcome-success")} role="status">
          Premium activ. Spor la invatat!
        </div>
      ) : null}
      {locked ? <div className={moduleClassNames([styles], "mode-lock-banner")} role="status"><strong>Plan activ necesar</strong><p>{lockMessage}</p></div> : null}
      {!locked ? (
        <div className={moduleClassNames([styles], "mode-grid")} aria-label="Alege modul de invatare">
          <LearningModeCard
        ...
        <div className={moduleClassNames([styles], "mode-lock-actions")}>
          <p>Modurile de invatare se deschid dupa activare.</p>
          <PendingNavigationLink className={moduleClassNames([styles], "btn-link")} href={lockHref} pendingLabel="Se deschid planurile...">
            Alege un plan

...
export function AdminDictionaryPanel({ categories = [], terms = [], runs = [], automationSettings, warning }) {
  ...
  return (
    <section className="surface admin-dictionary-panel">
      <div className="admin-dictionary-head">
        <div><span className="ui-section-label">Dicționar public</span><h2>Termeni, publicare și istoric</h2><p>Editezi conținutul înainte de publicare sau retragi o pagină fără să pierzi istoricul.</p></div>
      ...
      <div className="admin-dictionary-grid">
        <div className="admin-dictionary-list">
          {terms.map((term) => <button type="button" key={term.id} className={term.id === selectedId ? "is-selected" : ""} onClick={() => select(term)}><BookOpenCheck size={16} /><span><strong>{term.term}</strong><small>{term.status === "published" ? "Publicat" : term.status}</small></span><em>{term.quality_score}/100</em></button>)}
        </div>
      ...
      <div className="admin-dictionary-runs"><h3>Ultimele rulări</h3>{runs.length ? runs.map((run) => <article key={run.id}><strong>{run.candidate_term || "Fără termen"}</strong><span>{run.trigger_source === "cron" ? "Programat" : "Manual"} · {run.status} · {run.model || "model necunoscut"}</span>{run.error_message ? <small>{run.error_message}</small> : null}</article>) : <p>Nu există rulări încă.</p>}</div>
    </section>
  );
}

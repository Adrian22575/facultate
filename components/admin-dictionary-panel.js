...
function ActionMessage({ message }) {
  ...
  return <p className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], `admin-dictionary-action-message is-${message.tone || "info"}`)} role="status" aria-live="polite" aria-atomic="true">{message.text}</p>;
}
...
export function AdminDictionaryPanel({
  ...
  return (
    <section className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], `surface admin-dictionary-panel${detail ? " is-detail" : ""}`)}>
      {detail ? (
        <header className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-detail-head")}>
          <Link href="/admin/continut/dictionar" className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "btn-back")}><ArrowLeft size={16} aria-hidden="true" />Înapoi la termeni</Link>
        ...
        <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-content-toolbar")}>
          <AdminEditorialAutomationSettings workflow="dictionary" settings={automationSettings} generationPreview={generationPreview} />
          <button type="button" className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "btn-link")} onClick={generate} disabled={Boolean(busy) || Boolean(activeRun) || dirty} title={dirty ? "Salvează modificările înainte de a genera alt termen." : undefined}>
      ...
      {!detail && liveRun ? (
        <section className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-live-run")} aria-live="polite">
          ...
          <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-live-progress")} aria-label={`Progres estimat ${RUN_PROGRESS[liveRun.status] || 12}%`}>
      ...
      {warning ? <p className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-message is-error")}>{warning}</p> : null}
      {!detail ? <section className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], `admin-dictionary-schedule-status is-${scheduleState.tone}`)} aria-live="polite">
        <Clock3 size={19} aria-hidden="true" />
        <div><span>Automatizare dicționar</span><strong>{scheduleState.title}</strong><small>{scheduleState.detail}</small></div>
      </section> : null}

      <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], `admin-dictionary-grid${detail ? " is-detail" : ""}`)}>
        {!detail ? <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list")}>
          <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-tools")}>
            <FilterSearch
              value={termQuery}
              ...
              compact
              loading={searchBusy}
              className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-search")}
              inputProps={{ autoComplete: "off" }}
            />
            <p className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-count")} aria-live="polite">{termQuery.trim().length >= 2 ? searchBusy ? "Căutăm în dicționar…" : searchError ? "Căutarea nu este disponibilă" : `${visibleTerms.length} rezultate` : `${terms.length} termeni recenți`}</p>
          </div>
          {visibleTerms.map((term) => {
            const displayed = { ...term, ...(termPatches[term.id] || {}) };
            const displayedStatus = termStatus(displayed.status);
            return (
              <button type="button" key={term.id} className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], `admin-dictionary-list-item${term.id === selectedId ? " is-selected" : ""}`)} onClick={() => select(displayed)} disabled={Boolean(busy)}>
                <BookOpenCheck size={16} />
                <span className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-copy")}>
                  <strong>{displayed.term}</strong>
                  <small className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-meta")}><b>{displayedStatus.label}</b><i aria-hidden="true">·</i> Calitate {displayed.quality_score ?? "—"}/100</small>
                  <time className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-created")} dateTime={displayed.created_at || undefined} title={`Creat la ${formatDateTime(displayed.created_at)}`}>
          ...
          {!searchBusy && !searchError && visibleTerms.length === 0 ? <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-empty")}><strong>Niciun termen găsit</strong><span>Încearcă o formulare mai scurtă sau fără semne speciale.</span></div> : null}
          {searchError ? <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-list-empty is-error")}><strong>Căutarea nu a răspuns</strong><span>Termenii recenți rămân disponibili. Încearcă din nou.</span></div> : null}
        </div> : null}

        {effectiveSelected && form ? (
          <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-editor")} aria-busy={Boolean(busy)} inert={busy ? true : undefined}>
            <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-statebar")}>
              <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], `is-${statusInfo.tone}`)}><span>Stare</span><strong>{statusInfo.label}</strong><small>{statusInfo.help}</small></div>
              <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], score >= 82 ? "is-passed" : "is-failed")}><span>Calitate</span><strong>{dirty ? "Modificări nesalvate" : `${score}/100`}</strong><small>{dirty ? "Salvează pentru recalcularea scorului." : score >= 82 ? "Pragul editorial este îndeplinit." : "Pragul de publicare este 82."}</small></div>
              <div><span>Categorie</span><strong>{selectedCategory}</strong><small>Poate fi schimbată înainte de publicare.</small></div>
            </div>

            <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-row is-primary")}>
            ...
            <details className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-content-details")}>
              ...
              <div>
                ...
                <label>Întrebări frecvente <small>exact trei întrebări, în format JSON</small><textarea className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-json")} value={form.faqs} onChange={(event) => setField("faqs", event.target.value)} /></label>
            ...
            <section className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-workflow")} aria-labelledby="dictionary-workflow-title">
              ...
              <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-workflow-steps")}>
                <article className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], score >= 82 && !dirty ? "is-passed" : "")}>
                  ...
                  <span className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-score")}>{dirty ? "—" : score}/100</span>
                </article>
      ...
      {!detail ? <details className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-run-history")} open={runs.some((run) => run.status === "failed")}>
        <summary>Istoric generări ({runs.length})</summary>
        {runs.length ? <div className={moduleClassNames([panelStyles, listStyles, workflowStyles, libraryStyles], "admin-dictionary-runs")}>{runs.map((run) => <article key={run.id}><strong>{run.candidate_term || "Fără termen"}</strong><span>{run.trigger_source === "cron" ? "Programat" : "Manual"} · {runStatusLabel(run.status)} · {run.model || "model necunoscut"}</span><small>{run.quality_score == null ? "Scor indisponibil" : `${run.quality_score}/100`}</small>{run.rejection_reason || run.error_message ? <small>{run.rejection_reason || run.error_message}</small> : null}</article>)}</div> : <p>Nu există rulări încă.</p>}
      </details> : null}
    </section>
  );
}

...
import linkedInStyles from "./admin-linkedin-distribution.module.css";
...
export function AdminLinkedInDistribution({ data, article, initialPostId = "" }) {
  ...
  return (
    <section className={moduleClassNames(linkedInStyles, "admin-linkedin-article")} aria-labelledby="linkedin-article-title">
      <header className={moduleClassNames(linkedInStyles, "admin-linkedin-article-head")}><div className={moduleClassNames(linkedInStyles, "admin-linkedin-title-mark")}><span className={moduleClassNames(linkedInStyles, "admin-linkedin-brand-glyph")} aria-hidden="true">in</span></div><div><span>Distribuire editorială</span><h3 id="linkedin-article-title">Postări LinkedIn</h3><p>Fiecare variantă păstrează strategia, scorul și istoricul editărilor.</p></div><span className={moduleClassNames(linkedInStyles, `admin-linkedin-connection-state is-${connected ? "connected" : "offline"}`)}>{connected ? "Conectat" : connection?.status === "connection_expired" ? "Expirat" : "Neconectat"}</span></header>
      ...
      {data?.warning ? <p className={moduleClassNames(linkedInStyles, "admin-linkedin-message is-error")}>{data.warning}</p> : null}
      {message ? <p className={moduleClassNames(linkedInStyles, `admin-linkedin-message is-${messageTone}`)} role="status" aria-live="polite">{message}</p> : null}

      {article?.status !== "published" ? <div className={moduleClassNames(linkedInStyles, "admin-linkedin-empty")}><strong>Articolul nu este publicat încă</strong><p>Publică articolul mai întâi, apoi pregătește postarea.</p></div> : <>
        <section className={moduleClassNames(linkedInStyles, "admin-linkedin-generator")}><div><span>Strategia următoarei variante</span><strong>Alege intenția. Sistemul selectează un singur unghi și verifică rezultatul.</strong></div><LinkedInGenerationOptions value={manualOptions} onChange={setManualOptions} disabled={!connected || Boolean(busy)} /><div className={moduleClassNames(linkedInStyles, "admin-linkedin-generator-footer")}><div className={moduleClassNames(linkedInStyles, "admin-linkedin-article-activity")}><strong>{articleActivity.published ? `Publicată de ${articleActivity.published} ori` : "Încă nepublicată pe LinkedIn"}</strong><span>O variantă nouă nu modifică postările publicate.</span></div><button type="button" className={moduleClassNames(linkedInStyles, "btn-link")} onClick={generate} disabled={!canPrepare || Boolean(busy)}>{busy === "generate" ? <LoaderCircle className={moduleClassNames(linkedInStyles, "is-spinning")} size={16} /> : <FileText size={16} />}{busy === "generate" ? "Se pregătește…" : articleActivity.total ? "Creează variantă nouă" : "Pregătește postarea"}</button></div></section>
        <AdminGenerationPromptPreview preview={promptPreview ? { ...promptPreview, model: settings.model } : null} />

        {articlePosts.length ? <div className={moduleClassNames(linkedInStyles, "admin-linkedin-workspace is-article-context")}><div className={moduleClassNames(linkedInStyles, "admin-linkedin-list")} role="group" aria-label="Variantele postării LinkedIn">{articlePosts.map((post) => { const status = STATUS[post.status] || [post.status, "draft"]; return <button type="button" key={post.id} className={moduleClassNames(linkedInStyles, post.id === selected?.id ? "is-selected" : "")} onClick={() => choose(post)} disabled={Boolean(busy)} aria-pressed={post.id === selected?.id}><span className={moduleClassNames(linkedInStyles, `is-${status[1]}`)}>{status[0]}</span><strong>Varianta {post.edition_number || 1}</strong><small>{post.quality_score == null ? formatDate(post.updated_at) : `Scor ${Number(post.quality_score).toFixed(1)} · ${formatDate(post.updated_at)}`}</small></button>; })}</div>
          {selected ? <div className={moduleClassNames(linkedInStyles, "admin-linkedin-editor")} aria-busy={Boolean(busy)}>
            <div className={moduleClassNames(linkedInStyles, "admin-linkedin-editor-head")}><div><span className={moduleClassNames(linkedInStyles, `admin-linkedin-status is-${selectedStatus[1]}`)}>{selectedStatus[0]}</span><small className={moduleClassNames(linkedInStyles, "admin-linkedin-template-chip")}>Varianta {selected.edition_number || 1} · {LINKEDIN_POST_TEMPLATES.find((item) => item.key === selected.template_key)?.label || "Format salvat"}</small><h3>{finalPayload.angle || article.title}</h3></div>{selected.status === "published" && selected.linkedin_post_url ? <a href={selected.linkedin_post_url} target="_blank" rel="noreferrer">Deschide pe LinkedIn <ExternalLink size={15} /></a> : null}</div>
            {quality != null ? <div className={moduleClassNames(linkedInStyles, "admin-linkedin-quality")}><span><strong>{quality.toFixed(1)}</strong>/10</span><div><strong>{quality >= 8 ? "Trece pragul editorial" : "Necesită atenție"}</strong><small>Hook, claritate, specificitate, autenticitate și fidelitate față de articol.</small></div></div> : null}
            {selected.generation_warnings?.length ? <details className={moduleClassNames(linkedInStyles, "admin-linkedin-warnings")}><summary>{selected.generation_warnings.length} observații editoriale</summary><ul>{selected.generation_warnings.map((warning, index) => <li key={`${warning}-${index}`}>{warning}</li>)}</ul></details> : null}
            <div className={moduleClassNames(linkedInStyles, "admin-linkedin-editor-tabs")} role="tablist" aria-label="Editarea postării" onKeyDown={handleTablistKeyDown}><button id="linkedin-editor-tab-edit" type="button" role="tab" aria-selected={editorView === "edit"} aria-controls="linkedin-editor-panel" tabIndex={editorView === "edit" ? 0 : -1} className={moduleClassNames(linkedInStyles, editorView === "edit" ? "is-active" : "")} onClick={() => setEditorView("edit")}><FilePenLine size={15} />Editează</button><button id="linkedin-editor-tab-preview" type="button" role="tab" aria-selected={editorView === "preview"} aria-controls="linkedin-editor-panel" tabIndex={editorView === "preview" ? 0 : -1} className={moduleClassNames(linkedInStyles, editorView === "preview" ? "is-active" : "")} onClick={() => setEditorView("preview")}><Eye size={15} />Previzualizare</button></div>
            <div id="linkedin-editor-panel" role="tabpanel" aria-labelledby={`linkedin-editor-tab-${editorView}`}>{editorView === "edit" ? <label className={moduleClassNames(linkedInStyles, "admin-linkedin-text")}><span>Textul postării <small className={moduleClassNames(linkedInStyles, text.length > 2800 ? "is-warning" : "")}>{text.length}/3.000</small></span><textarea value={text} onChange={(event) => setText(event.target.value)} disabled={["publishing", "published"].includes(selected.status) || Boolean(busy)} rows={14} /></label> : <article className={moduleClassNames(linkedInStyles, "linkedin-post-preview")}><header><span className={moduleClassNames(linkedInStyles, "admin-linkedin-brand-glyph")}>in</span><div><strong>{connection?.display_name || "Profil LinkedIn"}</strong><small>Acum · Vizibil pentru oricine</small></div></header><p>{text}</p><footer><span>Apreciază</span><span>Comentează</span><span>Distribuie</span><span>Trimite</span></footer></article>}</div>
            {text.length > 2800 ? <p className={moduleClassNames(linkedInStyles, "admin-linkedin-length-warning")}>Te apropii de limita LinkedIn. Mai ai {Math.max(0, 3000 - text.length)} caractere.</p> : null}
            {selected.last_error ? <p className={moduleClassNames(linkedInStyles, "admin-linkedin-error")}><XCircle size={16} />{humanError(selected.last_error)}</p> : null}
            {selected.link_comment_error ? <p className={moduleClassNames(linkedInStyles, "admin-linkedin-error")}><XCircle size={16} />{humanError(selected.link_comment_error)}</p> : null}
            {!['published', 'publishing', 'not_generated'].includes(selected.status) ? <details className={moduleClassNames(linkedInStyles, "admin-linkedin-refinements")}><summary><WandSparkles size={15} />Rafinează varianta</summary><div>{REFINEMENTS.map(([key, label]) => <button type="button" key={key} className={moduleClassNames(linkedInStyles, "admin-linkedin-secondary")} onClick={() => action(key)} disabled={Boolean(busy)}>{busy === key ? <LoaderCircle className={moduleClassNames(linkedInStyles, "is-spinning")} size={15} /> : null}{label}</button>)}</div></details> : null}
            <div className={moduleClassNames(linkedInStyles, "admin-linkedin-actions")}>{!["published", "publishing", "not_generated"].includes(selected.status) ? <button type="button" className={moduleClassNames(linkedInStyles, "btn-back admin-linkedin-secondary")} onClick={saveText} disabled={!textDirty || Boolean(busy)}><Save size={16} />{textDirty ? "Salvează textul" : "Text salvat"}</button> : null}{["draft", "pending_approval", "rejected", "failed"].includes(selected.status) && !publishAmbiguous ? <button type="button" className={moduleClassNames(linkedInStyles, "admin-linkedin-approve")} onClick={() => action("approve")} disabled={textDirty || Boolean(busy)}><CheckCircle2 size={16} />Aprobă</button> : null}{["draft", "pending_approval", "approved", "failed"].includes(selected.status) && !publishAmbiguous ? <button type="button" className={moduleClassNames(linkedInStyles, "admin-linkedin-reject")} onClick={() => action("reject")} disabled={Boolean(busy)}><XCircle size={16} />Respinge</button> : null}{selected.status === "approved" ? <button type="button" className={moduleClassNames(linkedInStyles, "btn-link")} onClick={() => action("publish")} disabled={Boolean(busy)}><Send size={16} />{busy === "publish" ? "Se publică…" : "Publică pe LinkedIn"}</button> : null}{selected.status === "failed" && !publishAmbiguous ? <button type="button" className={moduleClassNames(linkedInStyles, "btn-link")} onClick={() => action("retry")} disabled={Boolean(busy)}><RefreshCw size={16} />Reîncearcă</button> : null}{selected.status === "published" && selected.link_comment_status === "failed" && !commentAmbiguous ? <button type="button" className={moduleClassNames(linkedInStyles, "admin-linkedin-secondary")} onClick={() => action("retry_comment")} disabled={Boolean(busy)}>Reîncearcă primul comentariu</button> : null}</div>
            <div className={moduleClassNames(linkedInStyles, "admin-linkedin-feedback")}><span>A fost utilă varianta?</span><button type="button" className={moduleClassNames(linkedInStyles, selected.feedback === "up" ? "is-active" : "")} onClick={() => action("feedback", { feedback: "up" })} disabled={Boolean(busy)} aria-label="Feedback pozitiv"><ThumbsUp size={15} /></button><button type="button" className={moduleClassNames(linkedInStyles, selected.feedback === "down" ? "is-active" : "")} onClick={() => action("feedback", { feedback: "down" })} disabled={Boolean(busy)} aria-label="Feedback negativ"><ThumbsDown size={15} /></button></div>
          ...
          </div> : null}</div> : <div className={moduleClassNames(linkedInStyles, "admin-linkedin-empty")}><strong>Nicio postare pregătită</strong><p>Alege strategia și pregătește prima variantă.</p></div>}
      </>}
    </section>
  );
}

...
import linkedInEditorStyles from "./admin-linkedin-editor.module.css";
import linkedInPreviewStyles from "./admin-linkedin-preview.module.css";
...
export function AdminLinkedInDistribution({ data, article, initialPostId = "" }) {
  ...
  return (
    <section className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-article")} aria-labelledby="linkedin-article-title">
      <header className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-article-head")}><div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-title-mark")}><span className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-brand-glyph")} aria-hidden="true">in</span></div><div><span>Distribuire editorială</span><h3 id="linkedin-article-title">Postări LinkedIn</h3><p>Fiecare variantă păstrează strategia, scorul și istoricul editărilor.</p></div><span className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], `admin-linkedin-connection-state is-${connected ? "connected" : "offline"}`)}>{connected ? "Conectat" : connection?.status === "connection_expired" ? "Expirat" : "Neconectat"}</span></header>
      ...
      {article?.status !== "published" ? <div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-empty")}><strong>Articolul nu este publicat încă</strong><p>Publică articolul mai întâi, apoi pregătește postarea.</p></div> : <>
        ...
        {articlePosts.length ? <div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-workspace is-article-context")}><div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-list")} role="group" aria-label="Variantele postării LinkedIn">{articlePosts.map((post) => { const status = STATUS[post.status] || [post.status, "draft"]; return <button type="button" key={post.id} className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], post.id === selected?.id ? "is-selected" : "")} onClick={() => choose(post)} disabled={Boolean(busy)} aria-pressed={post.id === selected?.id}><span className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], `is-${status[1]}`)}>{status[0]}</span><strong>Varianta {post.edition_number || 1}</strong><small>{post.quality_score == null ? formatDate(post.updated_at) : `Scor ${Number(post.quality_score).toFixed(1)} · ${formatDate(post.updated_at)}`}</small></button>; })}</div>
          {selected ? <div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-editor")} aria-busy={Boolean(busy)}>
            <div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-editor-head")}><div><span className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], `admin-linkedin-status is-${selectedStatus[1]}`)}>{selectedStatus[0]}</span><small className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-template-chip")}>Varianta {selected.edition_number || 1} · {LINKEDIN_POST_TEMPLATES.find((item) => item.key === selected.template_key)?.label || "Format salvat"}</small><h3>{finalPayload.angle || article.title}</h3></div>{selected.status === "published" && selected.linkedin_post_url ? <a href={selected.linkedin_post_url} target="_blank" rel="noreferrer">Deschide pe LinkedIn <ExternalLink size={15} /></a> : null}</div>
            {quality != null ? <div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-quality")}><span><strong>{quality.toFixed(1)}</strong>/10</span><div><strong>{quality >= 8 ? "Trece pragul editorial" : "Necesită atenție"}</strong><small>Hook, claritate, specificitate, autenticitate și fidelitate față de articol.</small></div></div> : null}
            {selected.generation_warnings?.length ? <details className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-warnings")}><summary>{selected.generation_warnings.length} observații editoriale</summary><ul>{selected.generation_warnings.map((warning, index) => <li key={`${warning}-${index}`}>{warning}</li>)}</ul></details> : null}
            <div className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-editor-tabs")} role="tablist" aria-label="Editarea postării" onKeyDown={handleTablistKeyDown}><button id="linkedin-editor-tab-edit" type="button" role="tab" aria-selected={editorView === "edit"} aria-controls="linkedin-editor-panel" tabIndex={editorView === "edit" ? 0 : -1} className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], editorView === "edit" ? "is-active" : "")} onClick={() => setEditorView("edit")}><FilePenLine size={15} />Editează</button><button id="linkedin-editor-tab-preview" type="button" role="tab" aria-selected={editorView === "preview"} aria-controls="linkedin-editor-panel" tabIndex={editorView === "preview" ? 0 : -1} className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], editorView === "preview" ? "is-active" : "")} onClick={() => setEditorView("preview")}><Eye size={15} />Previzualizare</button></div>
            <div id="linkedin-editor-panel" role="tabpanel" aria-labelledby={`linkedin-editor-tab-${editorView}`}>{editorView === "edit" ? <label className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-text")}><span>Textul postării <small className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], text.length > 2800 ? "is-warning" : "")}>{text.length}/3.000</small></span><textarea value={text} onChange={(event) => setText(event.target.value)} disabled={["publishing", "published"].includes(selected.status) || Boolean(busy)} rows={14} /></label> : <article className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "linkedin-post-preview")}><header><span className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-brand-glyph")}>in</span><div><strong>{connection?.display_name || "Profil LinkedIn"}</strong><small>Acum · Vizibil pentru oricine</small></div></header><p>{text}</p><footer><span>Apreciază</span><span>Comentează</span><span>Distribuie</span><span>Trimite</span></footer></article>}</div>
            {text.length > 2800 ? <p className={moduleClassNames([linkedInStyles, linkedInEditorStyles, linkedInPreviewStyles], "admin-linkedin-length-warning")}>Te apropii de limita LinkedIn. Mai ai {Math.max(0, 3000 - text.length)} caractere.</p> : null}

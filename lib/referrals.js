...
export async function activateReadyReferralReward({ userId }) {
  ...
  const { data: grantResult, error: grantError } = await admin.rpc("apply_reward_premium_grant", {
    p_user_id: userId,
    p_source: "referral",
    p_plan_code: REFERRAL_REWARD_PRODUCT_CODE,
    p_duration_hours: REFERRAL_REWARD_HOURS,
  ...
  const rewardGrantId = grantResult?.grantId || referral.reward_grant_id || null;

...
export async function createPendingLearningStudySet({
  ...
  const { data: studySet, error: studySetError } = await admin
    ...
    .insert({
      user_id: userId,
      subject_id: normalizeText(subjectId) || null,
...
export async function getUserLearningStudySets(userId, limit = 8) {
  ...
  const { data, error } = await admin
    .from("learning_study_sets")
    .select(
      "id, subject_id, title, status, visibility_scope, published_at, estimated_pages, chapter_count, flashcard_count, question_count, recommended_days, updated_at, created_at"
    )
    .eq("user_id", userId)
    .neq("status", "archived")
...
export async function getCommunityLearningStudySets({ userId, academicContext, limit = 8 }) {
  ...
  const { data, error } = await admin
    .from("learning_study_sets")
    .select(
      "id, user_id, subject_id, title, status, visibility_scope, target_cohort_id, target_unit_id, target_institution_id, published_at, estimated_pages, chapter_count, flashcard_count, question_count, recommended_days, updated_at, created_at"
    )
...
export async function getLearningStudySetsForSubject({ subjectId, userId, academicContext, limit = 6 }) {
  ...
  return (data || [])
    .filter((row) => row.user_id === userId || studySetMatchesCommunity(row, academicContext))
    .slice(0, limit)
    .map((row) => ({
      ...mapLearningStudySetSummary(row),
      isOwner: row.user_id === userId
    }));
}

export async function assignOwnedLearningStudySetSubject({ userId, studySetId, subjectId }) {
  const admin = createAdminClient();
  const { data, error } = await admin
    .from("learning_study_sets")
  ...
  if (error) throw error;
  if (!data) throw new Error("learning_study_set_not_found");
  return { studySetId: data.id, subjectId: data.subject_id };
}

...
function validateQuestionBankCorrectIndex(value, context) {
...
}).superRefine(validateQuestionBankCorrectIndex);

export const QuestionBankManualItemSchema = z.object({
  bankId: z.string().uuid(),
  questionText: z.string().min(10),
  answers: z.array(z.string().min(1)).min(4).max(5),
  correctIndex: z.coerce.number().int().min(0),
...
export const GenerateTestInputSchema = z
  .object({
    ...
    answerKeyPlacement: z.preprocess((value) => {
      if (value === null || value === undefined || value === "") {
        return undefined;
      }

      return value;
    }, z.enum(["unknown", "after_each_question", "at_end", "mixed"]).optional())
  })
  .superRefine((value, context) => {
    if (!value.answerKeyPlacement) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Alege unde sunt raspunsurile corecte.",
        path: ["answerKeyPlacement"]
      });
    }

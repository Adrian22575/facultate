...
const IMPORT_LOCK_STALE_MS = 8 * 60 * 1000;
...
const CHUNK_BOUNDARY_CONTEXT_CHARS = Number(process.env.AI_IMPORT_CHUNK_BOUNDARY_CONTEXT_CHARS || 3500);
const CARRY_OVER_FRAGMENT_MAX_CHARS = 4000;
...
function getTextHead(text, maxChars = CHUNK_BOUNDARY_CONTEXT_CHARS) {
  const value = cleanupText(text);
  return value.length > maxChars ? value.slice(0, maxChars).trim() : value;
}

function getTextTail(text, maxChars = CHUNK_BOUNDARY_CONTEXT_CHARS) {
  const value = cleanupText(text);
  return value.length > maxChars ? value.slice(-maxChars).trim() : value;
}
...
function normalizeCarryOverFragments(fragments = []) {
  ...
  return fragments
    .map((fragment) => {
      const text = cleanupText(fragment?.text || "").slice(0, CARRY_OVER_FRAGMENT_MAX_CHARS).trim();
      if (!text) {
        return null;
      }

      const placement = ["start", "end", "unknown"].includes(fragment?.placement)
        ? fragment.placement
        : "unknown";
      ...
      return {
        placement,
        text,
        reason: cleanupText(fragment?.reason || "").slice(0, 500)
      };
    })
    .filter(Boolean)
    .slice(0, 4);
...
function getChunkCarryOverFragments(chunk) {
  return normalizeCarryOverFragments(
    chunk?.metadata?.carryOverFragments ||
      chunk?.metadata?.carry_over_fragments ||
      chunk?.metadata?.carryOverContext ||
      []
  );
}

function formatCarryOverContext(chunk) {
  const fragments = getChunkCarryOverFragments(chunk);
  if (!fragments.length) {
    return "";
  }
  ...
  return fragments
    .map((fragment, index) => {
      const reason = fragment.reason ? `\nMotiv: ${fragment.reason}` : "";
      return `Fragment continuat ${index + 1} (${fragment.placement}):\n${fragment.text}${reason}`;
    })
    .join("\n\n");
}

function buildChunkExtractionContent({ title, chunk }) {
  const metadata = chunk.metadata || {};
  const parts = [];
  const carryOverContext = formatCarryOverContext(chunk);
  const hasFollowingChunk = Boolean(metadata.nextChunkHeadContext);
  const effectivePreviousChunkTailContext = carryOverContext ? "" : metadata.previousChunkTailContext;

  if (carryOverContext) {
    parts.push(
      "Fragmente reportate din chunkul anterior. Trateaza-le ca inceputul acestui chunk: extrage un fragment daca are intrebare si variante complete; combina-l cu inceputul fragmentului curent doar daca este incomplet:\n\n" +
        carryOverContext
    );
  }

  if (effectivePreviousChunkTailContext) {
    parts.push(
      "Context din finalul chunkului anterior. Foloseste-l doar daca inceputul fragmentului curent continua o intrebare sau un barem inceput anterior:\n\n" +
  ...
  if (hasFollowingChunk) {
    parts.push(
      "Acest chunk are continuare. Ultima intrebare sau ultimul bloc de raspunsuri din fragment trebuie returnat doar in carryOverFragments cu placement=end, nu in lista de intrebari sau baremuri extrase."
    );
  }
...
function attachBoundaryContextsToChunks(chunks) {
  return chunks.map((chunk, index) => ({
    ...chunk,
    metadata: {
      ...(chunk.metadata || {}),
      previousChunkTailContext: index > 0 ? getTextTail(chunks[index - 1]?.raw_text || "") : null,
      nextChunkHeadContext: index < chunks.length - 1 ? getTextHead(chunks[index + 1]?.raw_text || "") : null
    }
  }));
}
...
function normalizeExtractedOptions(rawOptions = []) {
  const sortedOptions = sortAnswerOptions(
    rawOptions.map((option, index) => {
      const label = normalizeLabel(option.label || expectedOptionLabel(index)) || expectedOptionLabel(index);
      return {
        label,
        text: stripAnswerLabelPrefix(cleanupText(option.text), label, index),
...
function buildQuestionHash(questionText, answers = [], correctAnswers = []) {
  const normalizedAnswers = (Array.isArray(answers) ? answers : [])
    .map((answer) => normalizeDuplicateText(answer))
    .filter(Boolean)
    .sort();
  ...
  return crypto
    .createHash("sha256")
    .update(
      `${normalizeDuplicateText(questionText)}::answers:${normalizedAnswers.join("|")}::correct:${normalizedCorrectAnswers.join("|")}`
    )
    .digest("hex");
}
...
function areLikelySameImportQuestionText(left, right) {
...
function scoreImportQuestionCandidate({ questionText, options = [], status = "missing_answer" }) {
  const optionChars = options.reduce((sum, option) => sum + cleanupText(option?.text || option).length, 0);
  const statusBonus = status === "answer_matched" ? 500 : status === "needs_review" ? -300 : 0;

  return cleanupText(questionText).length * 3 + optionChars + options.length * 30 + statusBonus;
}

function buildNormalizedImportAnswerSet(options = []) {
  return (Array.isArray(options) ? options : [])
    .map((option) => normalizeDuplicateText(option?.text || option))
    .filter(Boolean)
    .sort();
}

function buildNormalizedImportCorrectSet(options = []) {
  return (Array.isArray(options) ? options : [])
...
function areNormalizedImportListsEqual(left, right) {
...
function haveCompatibleImportAnswerOptions(leftOptions = [], rightOptions = []) {
  const leftAnswers = buildNormalizedImportAnswerSet(leftOptions);
  const rightAnswers = buildNormalizedImportAnswerSet(rightOptions);

  if (leftAnswers.length < 2 || rightAnswers.length < 2 || !areNormalizedImportListsEqual(leftAnswers, rightAnswers)) {
    return false;
  }

  const leftCorrect = buildNormalizedImportCorrectSet(leftOptions);
  const rightCorrect = buildNormalizedImportCorrectSet(rightOptions);

  return !leftCorrect.length || !rightCorrect.length || areNormalizedImportListsEqual(leftCorrect, rightCorrect);
}

export function areLikelySameImportQuestionForDedupe(left, right) {
  return (
    areLikelySameImportQuestionText(left?.questionText, right?.questionText) &&
    haveCompatibleImportAnswerOptions(left?.options || [], right?.options || [])
  );
}
...
function buildTextChunks(text, forcedClassification = null) {
  ...
  function pushCurrent({ deferTrailing = false } = {}) {
    const deferredTrailingBlock = deferTrailing && current.length > 1 ? current.pop() : null;
  ...
  for (const [index, block] of sourceBlocks.entries()) {
    ...
    if (current.length && nextLength > CHUNK_MAX_CHARS) {
      const deferredTrailingBlock = pushCurrent({ deferTrailing: true });
      current = deferredTrailingBlock ? [deferredTrailingBlock] : [];
      currentLength = deferredTrailingBlock ? deferredTrailingBlock.length + 2 : 0;
    }

    current.push(block);
    currentLength += block.length + 2;

    if (currentLength >= CHUNK_TARGET_CHARS && index < sourceBlocks.length - 1) {
      const deferredTrailingBlock = pushCurrent({ deferTrailing: true });
      current = deferredTrailingBlock ? [deferredTrailingBlock] : [];
      currentLength = deferredTrailingBlock ? deferredTrailingBlock.length + 2 : 0;
    }
  ...
  return chunks.length ? attachBoundaryContextsToChunks(chunks) : [];
}
...
function mapLicentaSessionForUi(session, jobs = [], resultBank = null) {
  ...
  const resultHref =
    session.status === "completed" && session.result_bank_id && resultBank?.status === "published"
      ...
      : reviewHref;
  const sourceDocumentId = session.source_document_id || resultBank?.source_document_id || null;
  ...
  return {
    ...
    sourceDocumentName:
      session.metadata?.originalSourceFilename ||
      session.metadata?.sourceFilename ||
      resultBank?.metadata?.originalSourceFilename ||
      null,
...
async function getLicentaResultBanksById({ admin, userId, bankIds }) {
  ...
  const { data, error } = await admin
    ...
    .select("id, status, source_document_id, metadata")
    .eq("user_id", userId)
    .in("id", ids);
...
async function extractQuestionsFromChunk({ job, chunk }) {
  const hasCarryOverContext = getChunkCarryOverFragments(chunk).length > 0;
  const hasPreviousChunkTailContext = Boolean(chunk.metadata?.previousChunkTailContext);
  const hasNextChunkHeadContext = Boolean(chunk.metadata?.nextChunkHeadContext);
  const response = await runLoggedResponseParse({
    ...
    metadata: {
      ...
      chunkIndex: chunk.chunk_index,
      hasCarryOverContext,
      hasPreviousChunkTailContext,
      hasNextChunkHeadContext
    },
    request: {
      model: IMPORT_MODEL,
      ...
      input: [
        {
          ...
          content: [
            ...
            "Nu include numarul intrebarii in questionText si nu include litera variantei in textul variantei; pastreaza aceste marcaje doar in localNumber si label.",
            "Daca enuntul contine o lista interna numerotata, de tip `1) ...; 2) ...; 3) ...`, pastreaza lista in questionText si nu o transforma in variante de raspuns.",
            hasCarryOverContext
              ? "Fragmentele reportate din chunkul anterior sunt primele blocuri ale acestui fragment: extrage-le ca intrebari daca sunt complete; combina-le cu inceputul fragmentului curent doar daca sunt incomplete."
              : "",
            "Contextul anterior/urmator este auxiliar: foloseste-l doar pentru prima sau ultima intrebare rupta de marginea fragmentului.",
            "Nu extrage intrebari independente care apar doar in contextul anterior sau urmator.",
            hasNextChunkHeadContext
              ? "Acest fragment are continuare. Nu extrage ultima intrebare din fragment; pune ultimul bloc de intrebare in carryOverFragments cu placement=end, chiar daca pare complet."
              : "",
            "Daca o intrebare ramane incompleta la final, pune fragmentul in carryOverFragments cu placement=end.",
            "Marcheaza needsReview=true pentru intrebarile incomplete sau ambigue."
          ].filter(Boolean).join(" ")
        },
        {
          role: "user",
          content: buildChunkExtractionContent({ title: "Fragment cu intrebari", chunk })
        }
      ],
      ...
      input: [
        {
          ...
          content: [
            ...
            hasCarryOverContext
              ? "Fragmentele reportate din chunkul anterior sunt primele blocuri ale acestui fragment: extrage-le ca barem daca sunt complete; combina-le cu inceputul fragmentului curent doar daca sunt incomplete."
              : "",
            "Contextul anterior/urmator este auxiliar: foloseste-l doar pentru un barem rupt de marginea fragmentului.",
            "Nu extrage baremuri independente care apar doar in contextul anterior sau urmator.",
            hasNextChunkHeadContext
              ? "Acest fragment are continuare. Nu extrage ultimul bloc de barem din fragment; pune-l in carryOverFragments cu placement=end, chiar daca pare complet."
              : "",
            "Daca un barem ramane incomplet la final, pune fragmentul in carryOverFragments cu placement=end."
          ].filter(Boolean).join(" ")
        },
        {
          role: "user",
          content: buildChunkExtractionContent({ title: "Fragment cu posibile raspunsuri", chunk })
        }
      ],
      ...
      input: [
        {
          ...
          content: [
            ...
            "Daca enuntul contine o lista interna numerotata, de tip `1) ...; 2) ...; 3) ...`, pastreaza lista in questionText si nu o transforma in variante de raspuns.",
            hasCarryOverContext
              ? "Fragmentele reportate din chunkul anterior sunt primele blocuri ale acestui fragment: extrage-le daca sunt complete; combina-le cu inceputul fragmentului curent doar daca sunt incomplete."
              : "",
            "Contextul anterior/urmator este auxiliar: foloseste-l doar pentru prima sau ultima intrebare ori barem rupt de marginea fragmentului.",
            "Nu extrage intrebari sau baremuri independente care apar doar in contextul anterior sau urmator.",
            hasNextChunkHeadContext
              ? "Acest fragment are continuare. Nu extrage ultima intrebare sau ultimul bloc de barem din fragment; pune-l in carryOverFragments cu placement=end, chiar daca pare complet."
              : "",
            "Daca un fragment ramane incomplet la final, pune-l in carryOverFragments cu placement=end."
          ].filter(Boolean).join(" ")
        },
        {
          role: "user",
          content: buildChunkExtractionContent({ title: "Material complet al setului", chunk })
        }
      ],
...
async function insertExtractedQuestions({ job, chunk, extraction }) {
  ...
  const { data: existingRows, error: existingError } = await admin
    .from("ai_import_questions")
    .select("id, global_index, question_text, status, metadata")
    .eq("import_job_id", job.id);
  ...
  const existingQuestionIds = (existingRows || []).map((row) => row.id).filter(Boolean);
  const { data: existingOptionRows, error: existingOptionsError } = existingQuestionIds.length
    ? await admin
  ...
  for (const option of existingOptionRows || []) {
    const current = existingOptionsByQuestionId.get(option.question_id) || [];
    current.push(option);
    existingOptionsByQuestionId.set(option.question_id, current);
  }
  ...
  const existingQuestions = (existingRows || [])
    .map((row) => ({
      ...
      status: row.status,
      hash: row?.metadata?.hash || null,
      options: existingOptionsByQuestionId.get(row.id) || []
    }))
    .filter((row) => row.id);
  ...
  for (const rawQuestion of extraction.questions || []) {
    ...
    const cleanedQuestionText = stripQuestionNumberPrefix(cleanupText(rawQuestion.questionText));

    if (!cleanedQuestionText || options.length < 2) {
      continue;
    }
    ...
    const status = rawQuestion.needsReview
      ...
      : hasUsableInlineAnswer
        ? "answer_matched"
        : "missing_answer";
    const candidateOptions = options.map((option) => ({
      label: option.label,
      text: option.text,
    ...
    const questionHash = buildQuestionHash(
      cleanedQuestionText,
      candidateOptions.map((option) => option.text),
      candidateOptions.filter((option) => option.is_correct).map((option) => option.text)
    );
    if (existingHashes.has(questionHash)) {
      continue;
    }

    const fuzzyDuplicateIndex = existingQuestions.findIndex((existingQuestion) =>
      areLikelySameImportQuestionForDedupe(existingQuestion, {
        questionText: cleanedQuestionText,
        options: candidateOptions
      })
    );
    let insertGlobalIndex = nextIndex;

    if (fuzzyDuplicateIndex >= 0) {
      const existingQuestion = existingQuestions[fuzzyDuplicateIndex];
      const existingScore = scoreImportQuestionCandidate({
        questionText: existingQuestion.questionText,
        options: existingQuestion.options,
        status: existingQuestion.status
      });
      const nextScore = scoreImportQuestionCandidate({
        questionText: cleanedQuestionText,
        options,
        status
      });
      ...
      if (existingQuestion.hash) {
        existingHashes.delete(existingQuestion.hash);
      }
      insertGlobalIndex = existingQuestion.globalIndex || nextIndex;
      existingQuestions.splice(fuzzyDuplicateIndex, 1);
    }
    ...
    const { data: question, error: questionError } = await admin
      ...
      .insert({
        ...
        global_index: insertGlobalIndex,
...
async function attachCarryOverFragmentsToNextChunk({ importJobId, currentChunkIndex, fragments }) {
  const forwardable = normalizeCarryOverFragments(fragments).filter((fragment) =>
    ["end", "unknown"].includes(fragment.placement)
  );
  ...
  const admin = createAdminClient();
  const { data: nextChunk, error } = await admin
    .from("ai_import_chunks")
    .select("id, metadata")
    .eq("import_job_id", importJobId)
  ...
  const existing = normalizeCarryOverFragments(nextChunk.metadata?.carryOverFragments || []);
  const merged = normalizeCarryOverFragments([...existing, ...forwardable]);
  const { error: updateError } = await admin
    .from("ai_import_chunks")
    .update({
      metadata: {
        ...(nextChunk.metadata || {}),
        carryOverFragments: merged
      }
    })
    .eq("id", nextChunk.id);
...
async function processNextChunk(job) {
  ...
  try {
    if (job.mode === "set" && chunk.metadata?.singlePassSet) {
      ...
      const carryOverFragments = normalizeCarryOverFragments(extraction.carryOverFragments || []);
    ...
    let savedAnswerKeys = 0;
    const carryOverFragments = [];
    const hasCarryOverContext = getChunkCarryOverFragments(chunk).length > 0;
    const shouldExtractQuestions =
      classification.classification === "questions" ||
      classification.classification === "mixed" ||
      hasCarryOverContext;
    const shouldExtractAnswerKey =
      classification.classification === "answer_key" ||
      classification.classification === "mixed";
    ...
    if (shouldExtractQuestions) {
      ...
      carryOverFragments.push(...normalizeCarryOverFragments(extraction.carryOverFragments || []));
    }

    if (shouldExtractAnswerKey) {
    ...
    await admin
      ...
      .update({
        ...
        metadata: {
          ...
          savedQuestions,
          savedAnswerKeys,
          carryOverFragments: normalizeCarryOverFragments(carryOverFragments)
        }
      })
      .eq("id", chunk.id);
...
export async function createSetImportJob({
  ...
  if (error) {
    if (error.code === "23505") {
      if (metadata?.requestId) {
        const { data: requestJob, error: requestJobError } = await admin
          .from("ai_import_jobs")
          .select("*")
          .eq("user_id", userId)
          .eq("metadata->>requestId", metadata.requestId)
          .maybeSingle();
        if (requestJobError) throw requestJobError;
        if (requestJob) return buildPublicStatus(requestJob);
      }
      ...
      if (existingJob) {
        if (
          metadata?.requestId &&
          existingJob.metadata?.requestId !== metadata.requestId &&
          allocationAttempt < 4
        ) {
          return createSetImportJob({
            userId,
            licentaSessionId: session.id,
            title,
            contentText,
  ...
  const rawChunks = unifiedChunks.length
    ? unifiedChunks
    : useSinglePassSet
      ? [
          {
...
async function acquireImportJobLock(importJobId) {
  const admin = createAdminClient();
  const staleBefore = new Date(Date.now() - IMPORT_LOCK_STALE_MS).toISOString();
  const { data, error } = await admin.rpc("acquire_ai_import_job_lock", {
...
async function releaseImportJobLock(importJobId) {
  const admin = createAdminClient();
  const { error } = await admin.rpc("release_ai_import_job_lock", {
    p_job_id: importJobId
  });
  if (error) throw error;
}
...
export async function claimNextImportJob() {
  ...
  const { data: candidates, error } = await admin
    .from("ai_import_jobs")
    .select("id, user_id, status, locked_at, updated_at")
    .in("status", [...IMPORT_ACTIVE_STATUSES])
    .order("updated_at", { ascending: true })
    .limit(12);
  if (error) throw error;

  for (const candidate of candidates || []) {
    if (await acquireImportJobLock(candidate.id)) {
      return {
        importJobId: candidate.id,
        userId: candidate.user_id,
        recovered: Boolean(candidate.locked_at)
      };
    }
  }
...
export async function releaseClaimedImportJob(importJobId) {
  await releaseImportJobLock(importJobId);
}

export async function processImportJob({ importJobId, userId, lockAlreadyAcquired = false }) {
  const locked = lockAlreadyAcquired || (await acquireImportJobLock(importJobId));
  if (!locked) {
  ...
  try {
    ...
    if (
      ["completed", "completed_with_warnings", "ready_for_preview", "needs_review"].includes(
        job.status
      )
    ) {
      return getImportStatus({ importJobId, userId });
    ...
    try {
      ...
      if (Number(refreshed.total_questions || 0) > 0) {
        ...
        await updateImportJob(importJobId, {
          status: message.toLowerCase().includes("pdf-ul pare scanat")
            ? "needs_review"
            : "failed",
          error_message: message,
...
export async function updateImportQuestion({ importJobId, questionId, userId, questionText, options, correctOptionIndex, markReviewed = false }) {
  ...
  const cleanedQuestion = stripQuestionNumberPrefix(cleanupText(questionText));
...
async function consumeImportCreditIfNeeded(job) {
  ...
  await consumeAIUploadCredit({
    ...
    insufficientMessage: "Nu ai incarcari disponibile pentru salvarea importului."
  });
...
async function updateImportOptionIfChanged({ admin, option, label, text, isCorrect }) {
  if (option.label === label && option.text === text && Boolean(option.is_correct) === Boolean(isCorrect)) {
    return;
  }
...
async function collectAcceptedImportItems(jobs) {
  ...
  for (const question of questions) {
    ...
    const cleanedQuestionText = stripQuestionNumberPrefix(cleanupText(question.question_text));
    ...
    items.push({
      ...
      explanation: "",
      normalized_hash: buildQuestionHash(
        cleanedQuestionText,
...
async function consumeLicentaSessionCreditIfNeeded(session) {
  ...
  await consumeAIUploadCredit({
    ...
    idempotencyKey: `licenta:${session.id}`,
    metadata: {
      licentaSessionId: session.id
    },
    insufficientMessage: "Nu ai incarcari disponibile pentru finalizarea licentei."
  });
...
export async function finalizeLicentaImportSession({ sessionId, userId }) {
  ...
  const finalSourceDocumentId =
    ...
    completedJobs.find((job) => job.source_document_id)?.source_document_id ||
    null;
  ...
  if (existingBank?.id) {
    ...
    const { error: updateBankError } = await admin
      .from("ai_question_banks")
      .update({
        source_document_id: finalSourceDocumentId,
        question_count: items.length,
        metadata: {
...
export async function retryImportJob({ importJobId, userId }) {
  ...
  const job = await fetchImportJob(importJobId, userId);
  if (job.status !== "failed") {
    throw new Error("Importul poate fi reluat doar dupa ce procesarea s-a oprit.");
  }
  ...
  const { count: chunkCount, error: chunkCountError } = await admin
    .from("ai_import_chunks")
    .select("id", { count: "exact", head: true })
    .eq("import_job_id", importJobId);
  ...
  const updated = await updateImportJob(job.id, {
    status: Number(chunkCount || 0) > 0 ? "processing" : "uploaded",
    error_message: null,
    completed_at: null
  });

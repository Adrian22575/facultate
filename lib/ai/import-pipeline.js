...
async function insertExtractedQuestions({ job, chunk, extraction }) {
  ...
  for (const rawQuestion of extraction.questions || []) {
    ...
    const questionHash = buildQuestionHash(rawQuestion.questionText, options.map((option) => option.text));
    ...
    const fuzzyDuplicateIndex = existingQuestions.findIndex((existingQuestion) =>
      areLikelySameImportQuestionText(existingQuestion.questionText, rawQuestion.questionText)
    );
    ...
    existingQuestions.push({
      ...
      questionText: cleanupText(rawQuestion.questionText),
      status,
      hash: questionHash
    });
  }

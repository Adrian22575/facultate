...
function buildQuestionHash(questionText, answers = [], correctIndex = 0) {
  const normalizedAnswers = (Array.isArray(answers) ? answers : [])
    .map((answer) => normalizeQuestionText(answer))
    .filter(Boolean)
    .sort();
  const normalizedCorrectAnswer = normalizeQuestionText(answers?.[correctIndex] || "");
  const base = `${normalizeQuestionText(questionText)}::answers:${normalizedAnswers.join("|")}::correct:${normalizedCorrectAnswer}`;
...
async function extractChunkItems({
  ...
  const instructions = [
    ...
    "Nu include numarul intrebarii in question_text si nu include litera variantei in textul raspunsului; pastreaza doar continutul semantic.",
    "Daca enuntul contine o lista interna numerotata, de tip `1) ...; 2) ...; 3) ...`, pastreaza lista in question_text si nu o transforma in variante de raspuns.",
...
function normalizeChunkItemDetailed(item, sourceChunkId, options = {}) {
  ...
  const questionText = stripQuestionNumberPrefix(cleanupText(item.question_text));
  const answers = (item.answers || [])
    .map((answer, index) => stripAnswerLabelPrefix(cleanupText(answer), "", index))
    .filter(Boolean);
...
function buildNormalizedAnswerSet(answers = []) {
...
function areNormalizedListsEqual(left, right) {
...
function haveSameAnswerSet(left, right) {
  const leftAnswers = buildNormalizedAnswerSet(left?.answers || []);
  const rightAnswers = buildNormalizedAnswerSet(right?.answers || []);

  if (leftAnswers.length < 2 || rightAnswers.length < 2) {
    return false;
  }

  return areNormalizedListsEqual(leftAnswers, rightAnswers);
}

function haveSameCorrectAnswer(left, right) {
  const leftAnswers = Array.isArray(left?.answers) ? left.answers : [];
  const rightAnswers = Array.isArray(right?.answers) ? right.answers : [];
  const leftCorrect = normalizeQuestionText(leftAnswers[left?.correct_index] || "");
  const rightCorrect = normalizeQuestionText(rightAnswers[right?.correct_index] || "");

  return Boolean(leftCorrect && rightCorrect && leftCorrect === rightCorrect);
}

export function areLikelySameQuestionBankItemForDedupe(left, right) {
  return (
    areLikelySameQuestionText(left?.question_text, right?.question_text) &&
    haveSameAnswerSet(left, right) &&
    haveSameCorrectAnswer(left, right)
  );
}

function findLikelyDuplicateQuestionIndex(items, candidate) {
  return items.findIndex((item) => areLikelySameQuestionBankItemForDedupe(item, candidate));
}

...
export async function applyAllowlistGrantsForExistingUsers(emails) {
  const normalizedEmails = Array.from(
    new Set((Array.isArray(emails) ? emails : []).map(normalizeEmail).filter(Boolean))
  );
  if (!normalizedEmails.length) {
    return { matchedUsers: 0, applied: 0 };
  }
  ...
  for (const profile of profiles || []) {
    const result = await ensurePremiumGrantFromAllowlist({
      userId: profile.id,
      email: profile.email
    });
    if (result.applied) {
  ...
  return {
    matchedUsers: profiles?.length || 0,
    applied
  };
}

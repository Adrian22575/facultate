...
export async function getDictionaryAdminOverview() {
  ...
  const [{ data: categories, error: categoriesError }, { data: terms, error: termsError }, { data: runs, error: runsError }, { data: automationSettings, error: automationError }] = await Promise.all([
    admin.from("dictionary_categories").select("id, slug, name, description, sort_order").order("sort_order").order("name"),
  ...
  return { categories: categories || [], terms: (terms || []).map(mapTerm), runs: runs || [], automationSettings: automationSettings || null, warning: null };
}

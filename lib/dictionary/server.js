...
import { getDictionaryGenerationPreview } from "@/lib/dictionary/automation";
...
export async function searchDictionaryAdminTerms(search, { limit = 60 } = {}) {
  const admin = getAdmin();
  const normalized = normalizeDictionaryText(search).slice(0, 120);
  if (!admin || normalized.length < 2) return [];
  const slug = dictionarySlug(normalized);
  const filters = [`term.ilike.%${normalized}%`];
  if (slug) filters.push(`slug.ilike.%${slug}%`);
  const { data, error } = await admin
    .from("dictionary_terms")
    .select(FULL_TERM_COLUMNS)
    .or(filters.join(","))
...
export async function getDictionaryAdminTermById(termId) {
  const admin = getAdmin();
  if (!admin) return null;
  const { data, error } = await admin
    .from("dictionary_terms")
...
export async function getDictionaryAdminEditorWorkspace(termId) {
  const admin = getAdmin();
  if (!admin) return { categories: [], term: null };

  const [{ data: categories, error: categoriesError }, term] = await Promise.all([
    admin.from("dictionary_categories").select("id, slug, name, description, sort_order").order("sort_order").order("name"),
    getDictionaryAdminTermById(termId)
  ]);
  if (categoriesError) throw categoriesError;
  return { categories: categories || [], term };
}
...
export async function getDictionaryAdminOverview() {
  const admin = getAdmin();
  if (!admin) return { categories: [], terms: [], runs: [], automationSettings: null, warning: "Conexiunea de administrare nu este disponibilă." };

  const [{ data: categories, error: categoriesError }, { data: terms, error: termsError }, { data: runs, error: runsError }, { data: automationSettings, error: automationError }, { data: previewTerms, error: previewTermsError }] = await Promise.all([
    admin.from("dictionary_categories").select("id, slug, name, description, sort_order").order("sort_order").order("name"),
    admin.from("dictionary_terms").select(FULL_TERM_COLUMNS).order("updated_at", { ascending: false }).limit(120),
    admin.from("dictionary_generation_runs").select("id, run_key, run_date, trigger_source, status, model, candidate_term, published_term_id, quality_score, rejection_reason, error_message, notification_sent, started_at, finished_at").order("started_at", { ascending: false }).limit(16),
    admin.from("editorial_automation_settings").select("workflow, enabled, frequency_days, weekly_day, scheduled_hour, model, notify_telegram, last_scheduled_for, updated_at").eq("workflow", "dictionary").maybeSingle(),
    admin.from("dictionary_terms").select("term, synonyms").order("created_at", { ascending: false }).limit(500)
  ]);
  if (categoriesError) throw categoriesError;
  if (termsError) throw termsError;
  if (runsError) throw runsError;
  if (automationError) throw automationError;
  if (previewTermsError) throw previewTermsError;
  return {
    categories: categories || [],
    terms: (terms || []).map(mapTerm),
    runs: runs || [],
    automationSettings: automationSettings || null,
    generationPreview: getDictionaryGenerationPreview({ categories, existingTerms: previewTerms, model: automationSettings?.model }),
    warning: null
  };
}

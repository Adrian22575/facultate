...
export function getDictionaryGenerationPreview({ categories = [], existingTerms = [], model = "gpt-5.4" } = {}) {
  return {
    workflow: "dictionary",
    model,
    ...
    requests: [{
      ...
      title: "Termen nou",
      reasoning: "high",
      output: "dictionary_term (structură validată)",
      tools: [],
      developerPrompt: buildPrompt({ categories, existingTerms }),
      userPrompt: "Generează acum termenul nou pentru dicționar.",
      dynamicContext: "Categoriile și primii 240 de termeni existenți sunt incluse mai sus exact cum sunt la momentul previzualizării."
    }],
    retryNote: "Dacă prima variantă nu trece validarea, următoarea cerere include și motivele respingerii pentru corectare."
  };
}

async function generateTerm({ categories, existingTerms, retryReasons, model }) {
...
export async function runDictionaryGeneration({ triggerSource = "cron", runKey, date = todayInBucharest(), now = new Date() } = {}) {
  const admin = createAdminClient();
  const settings = await getAutomationSettings("dictionary", admin);
  if (triggerSource === "cron" && !isAutomationDue(settings, now)) return { ok: true, skipped: true, reason: settings.enabled ? "not_due" : "automation_disabled" };
  const model = settings.model;
  const resolvedRunKey = runKey || `${triggerSource}:${date}`;
  const { data: run, error: insertError } = await admin.from("dictionary_generation_runs").insert({ run_key: resolvedRunKey, run_date: date, trigger_source: triggerSource, status: "started", model }).select("id").maybeSingle();
  ...
  if (triggerSource === "cron") await markAutomationScheduled("dictionary", date, admin);
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      generated = await generateTerm({ categories: categories || [], existingTerms: existingTerms || [], retryReasons: rejectionReasons, model });
    ...
    if (!generated || !assessment || rejectionReasons.length) {
      ...
      if (settings.notify_telegram) await notifyDictionaryGenerationFailed({ runKey: resolvedRunKey, reason: rejectionReasons.join(" ") || "validare nereușită" });
      return { ok: false, skipped: false, reason: "validation_failed" };
    }
    const categoryId = categoryIdFor(generated, categories || []);
    await updateRun(admin, run.id, { status: "validated" });
    const { data: created, error: createdError } = await admin.from("dictionary_terms").insert(toDbRecord(generated, categoryId, assessment, model)).select("id, slug, term").single();
    ...
    const notification = settings.notify_telegram ? await notifyDictionaryPublished({ term: created.term, slug: created.slug, category: generated.category, qualityScore: assessment.score, runKey: resolvedRunKey }) : { sent: false, skipped: true };
    ...
    if (settings.notify_telegram) await notifyDictionaryGenerationFailed({ runKey: resolvedRunKey, reason: message }).catch(() => null);
    throw error;
  }
}

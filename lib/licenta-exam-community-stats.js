...
async function fetchScopeRows(admin, scope) {
...
export async function buildLicentaExamCommunityStats({
  ...
  for (const scope of scopes) {
    const rows = await fetchScopeRows(admin, scope);
...
export async function buildLicentaExamStatsDashboard({ admin, academicContext, userId }) {
  ...
  const personalScores = personalAttempts.map((attempt) => attempt.scorePercent);
  const communityScores = communityAttempts.map((attempt) => attempt.scorePercent);
  const latestAttempt = personalAttempts[0] || null;
  ...
  const rankStats = buildRankStats({ attempts: communityAttempts, userId });
  ...
  return {
    ...
    overview: {
      personalAttemptCount: personalAttempts.length,
      communityAttemptCount: communityAttempts.length,
    ...
    communityDistribution: buildDistribution(communityAttempts),
    personalDistribution: buildDistribution(personalAttempts),

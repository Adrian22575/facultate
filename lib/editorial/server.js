...
export async function getEditorialAdminOverview() {
  ...
  const [{ data: articles, error: articleError }, { data: runs, error: runError }, { data: automationSettings, error: automationError }] = await Promise.all([
    ...
    admin.from("editorial_generation_runs").select("id, run_key, run_date, week_start, week_end, trigger_source, status, model, article_id, candidate_count, source_count, topic_count, quality_score, rejection_reason, error_message, notification_sent, started_at, finished_at").order("started_at", { ascending: false }).limit(30),
    admin.from("editorial_automation_settings").select("workflow, enabled, frequency_days, model, notify_telegram, last_scheduled_for, updated_at").eq("workflow", "editorial").maybeSingle()
  ]);
  if (articleError) throw articleError;
  if (runError) throw runError;
  if (automationError) throw automationError;
  return { articles: articles || [], runs: runs || [], automationSettings: automationSettings || null, warning: null };
}

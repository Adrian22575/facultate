...
export async function getEditorialAdminArticleWorkspace(articleId) {
  const admin = adminClient();
  if (!admin) return { article: null, runs: [] };
  const [{ data: article, error: articleError }, { data: runs, error: runsError }] =
    await Promise.all([
      admin
        .from("editorial_articles")
        .select(FULL_COLUMNS)
      ...
      admin
        ...
        .select(
          "id, run_key, run_date, week_start, week_end, trigger_source, status, model, article_id, candidate_count, source_count, topic_count, quality_score, rejection_reason, error_message, validation_report, notification_sent, started_at, finished_at"
        )
  ...
  return { article: article || null, runs: runs || [] };
}

export async function searchEditorialAdminArticles(search, { limit = 80 } = {}) {
  const admin = adminClient();
  const value = String(search || "").trim().replace(/[\\%_]/g, "\\$&");
  if (!admin || value.length < 2) return [];
  const { data, error } = await admin
    .from("editorial_articles")
    .select(FULL_COLUMNS)
    .ilike("title", `%${value}%`)
...
export async function getEditorialAdminOverview() {
  ...
  const [{ data: articles, error: articleError }, { data: runs, error: runError }, { data: automationSettings, error: automationError }] = await Promise.all([
    ...
    admin.from("editorial_automation_settings").select("workflow, enabled, frequency_days, weekly_day, scheduled_hour, model, notify_telegram, last_scheduled_for, updated_at").eq("workflow", "editorial").maybeSingle()
  ]);
  if (articleError) throw articleError;

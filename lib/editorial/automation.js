...
import { notifyEditorialGenerationFailed, notifyEditorialPublished } from "@/lib/notifications/telegram";
...
async function researchWeek(week) {
...
async function makePlan({ research, week }) {
  const response = await runLoggedResponseParse({
    ...
    request: {
      model: EDITORIAL_MODEL,
      reasoning: { effort: "high" },
      input: [{ role: "developer", content: "Construiește un plan editorial românesc precis, calm și fără clickbait. Folosește numai pachetul de cercetare validat furnizat. Alege 3-5 subiecte, explică de ce contează pentru elevi și studenți și nu introduce fapte noi." }, { role: "user", content: cleanPromptData({ week, research }) }],
...
async function draftArticle({ plan, research, week }) {
  const response = await runLoggedResponseParse({
    ...
    request: {
      model: EDITORIAL_MODEL,
      reasoning: { effort: "high" },
      input: [{ role: "developer", content: "Redactează un articol editorial în română, 1200-2500 de cuvinte când materialul o permite, destinat elevilor, studenților și profesorilor. Folosește exclusiv datele din pachetul validat. Fiecare secțiune are nevoie de sourceIds existente; nu crea surse sau linkuri noi. Separă faptele de interpretare, spune limitele și evită promisiunile sau dramatizarea. Titlul trebuie să indice perioada ori tema concretă." }, { role: "user", content: cleanPromptData({ week, plan, research }) }],
...
async function checkFacts({ draft, research }) {
  const response = await runLoggedResponseParse({
    ...
    request: {
      model: EDITORIAL_MODEL,
      reasoning: { effort: "high" },
      input: [{ role: "developer", content: "Ești verificator editorial independent. Compară proiectul exclusiv cu cercetarea validată. Marchează orice afirmație factuală care nu este susținută de sourceIds, orice dată incorectă, exagerare, cauzalitate nejustificată, sursă lipsă sau posibilă invenție. Treci verificarea numai dacă nu există afirmații fără suport și conținutul este suficient de clar pentru public." }, { role: "user", content: cleanPromptData({ draft, research }) }],
...
function dbRecord({ draft, research, factCheck, assessment, week, publish }) {
  ...
  return {
    ...
    generation_model: EDITORIAL_MODEL,
    published_at: publish ? now : null,
    last_reviewed_at: now
  };
}

export async function runEditorialGeneration({ triggerSource = "cron", runKey, date = new Date(), publish = triggerSource === "cron", force = false } = {}) {
  if (triggerSource === "cron" && !force && !isSundayInBucharest(date)) return { ok: true, skipped: true, reason: "not_editorial_day" };
  const week = getEditorialWeek(date);
  const admin = createAdminClient();
  const resolvedRunKey = runKey || `${triggerSource}:${week.key}`;
  const { data: run, error: insertError } = await admin.from("editorial_generation_runs").insert({ run_key: resolvedRunKey, week_start: week.start, week_end: week.end, trigger_source: triggerSource, status: "started", model: EDITORIAL_MODEL }).select("id").maybeSingle();
  if (insertError?.code === "23505") return { ok: true, skipped: true, reason: "already_ran_this_week" };
  ...
  try {
    ...
    const rawResearch = await researchWeek(week);
    ...
    if (!finalResearch.valid) {
      ...
      await notifyEditorialGenerationFailed({ runKey: resolvedRunKey, reason: finalResearch.reasons.join(" ") || "cercetare insuficientă" });
      return { ok: false, skipped: false, reason: "research_validation_failed" };
    }

    const plan = await makePlan({ research: finalResearch, week });
    const draft = await draftArticle({ plan, research: finalResearch, week });
    ...
    const factCheck = await checkFacts({ draft, research: finalResearch });
    ...
    if (!assessment.valid) {
      await updateRun(admin, run.id, { finished_at: new Date().toISOString() });
      await notifyEditorialGenerationFailed({ runKey: resolvedRunKey, reason: assessment.reasons.join(" ") || "verificarea editorială nu a trecut" });
      return { ok: false, skipped: false, reason: "quality_threshold_failed", score: assessment.score };
    }

    const record = dbRecord({ draft, research: finalResearch, factCheck, assessment, week, publish });
    ...
    if (publish) notification = await notifyEditorialPublished({ article, week, runKey: resolvedRunKey });
    ...
    await notifyEditorialGenerationFailed({ runKey: resolvedRunKey, reason: message }).catch(() => null);
    throw error;
  }
}
...
export async function runEditorialFactCheck(article) {
  ...
  return checkFacts({ draft, research: { sources } });
}

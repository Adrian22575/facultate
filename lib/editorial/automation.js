...
function getRollingResearchWindow(week, days = 28) {
  const start = new Date(`${week.start}T12:00:00Z`);
  start.setUTCDate(start.getUTCDate() - days);
  return {
    ...week,
    start: start.toISOString().slice(0, 10),
    key: `${start.toISOString().slice(0, 10)}:${week.end}`
  };
}

function researchPrompt(week, { fallback = false } = {}) {
  const scope = fallback
    ? [
      "Prima selecție nu a avut suficiente linkuri verificabile. Refă cercetarea independent, fără să reutilizezi URL-uri nesigure.",
      "Folosește intervalul recent extins indicat pentru context și menționează în titlurile subiectelor data sau perioada relevantă; nu pretinde că toate informațiile sunt noutăți din această săptămână.",
      "Prioritizează comunicate, rapoarte, studii și pagini directe de la instituții. Caută explicit cel puțin o sursă din România, una europeană și una internațională, când există subiecte relevante."
    ].join("\n")
    : "Începe cu informațiile publicate sau desfășurate în săptămâna indicată.";
  ...
  return [
    ...
    "Caută România, Uniunea Europeană și internațional numai când este util publicului român. Preferă pagini directe de la Ministerul Educației, UEFISCDI, ARACIS, universități și școli, Comisia Europeană, Eurostat, OECD, UNESCO, Banca Mondială, reviste științifice și rapoarte instituționale.",
    ...
    "Folosește linkul direct către comunicat, raport, studiu sau știre, nu către pagina principală. Nu include forumuri, postări sociale sau agregatoare ca suport factual.",
    "Returnează cel puțin 10 surse și 6 subiecte candidate distincte. Fiecare subiect trebuie să aibă minimum două surse când afirmațiile sunt importante.",
    "Evaluează fiecare criteriu — relevance, importance, recency, credibility și usefulness — strict pe aceeași scară de la 0 la 10. O sursă oficială sau un studiu primar solid poate primi 9-10 la credibility; nu folosi alte scări.",
    scope,
    `Interval de cercetare: ${week.start}–${week.end}.`
  ].join("\n");
}
...
async function researchWeek(week, model, { fallback = false } = {}) {
  const response = await runLoggedResponseParse({
    requestScope: "editorial_weekly_research",
    metadata: { weekStart: week.start, weekEnd: week.end, strategy: fallback ? "recent_fallback" : "weekly", tool: "web_search" },
    request: {
      model,
      reasoning: { effort: "high" },
      tools: [{ type: "web_search" }],
      input: [{ role: "developer", content: researchPrompt(week, { fallback }) }, { role: "user", content: "Începe cercetarea și returnează numai date structurate." }],
...
async function validateAndVerifyResearch(rawResearch, week) {
  const research = validateResearch(rawResearch, week);
  const returnedSourceCount = rawResearch.sources.length;
  const eligibleSourceCount = research.sources.length;
  const eligibleTopicCount = research.topics.length;
  const urlChecks = await Promise.all(
    research.sources.map(async (source) => [source.url, await verifyEditorialSourceUrl(source.url)])
  );
  ...
  research.topics = research.topics.filter((topic) =>
    topic.sourceIds.some((id) => research.sources.some((source) => source.id === id))
  );
  const finalResearch = validateResearch(
    { sources: research.sources, candidateTopics: research.topics },
    week
  );
  ...
  return {
    ...
    report: {
      returnedSourceCount,
      candidateCount: rawResearch.candidateTopics.length,
      eligibleSourceCount,
      eligibleTopicCount,
      rejectedByScoreCount: Math.max(0, returnedSourceCount - eligibleSourceCount),
      rejectedUrlCount: Math.max(0, eligibleSourceCount - validUrls.size),
      validatedSourceCount: finalResearch.sources.length,
      verifiedUrlCount: validUrls.size,
      topicCount: finalResearch.topics.length,
      reasons: finalResearch.reasons
    }
  };
}
...
export function getEditorialGenerationPreview({ date = new Date(), model = "gpt-5.4" } = {}) {
  ...
  const fallbackWindow = getRollingResearchWindow(week);
  return {
    workflow: "editorial",
    model,
    ...
    requests: [
      ...
      },
      {
        ...
        title: "Cercetare alternativă",
        reasoning: "high",
        output: "editorial_research (structură validată)",
        tools: ["web_search"],
        developerPrompt: researchPrompt(fallbackWindow, { fallback: true }),
        userPrompt: "Se rulează numai când prima cercetare nu are suficiente surse verificabile.",
        dynamicContext: `Fereastra recentă de rezervă este ${fallbackWindow.start}–${fallbackWindow.end}; o ciornă rămâne blocată dacă nici această rundă nu trece validarea.`
      },
      {
        id: "plan",
...
export async function runEditorialGeneration({ triggerSource = "cron", runKey, date = new Date(), publish = false, force = false } = {}) {
  ...
  try {
    ...
    const initialRawResearch = await researchWeek(week, model);
    let researchAttempt = await validateAndVerifyResearch(initialRawResearch, week);
    const researchAttempts = [{
      strategy: "weekly",
      window: { start: week.start, end: week.end },
      ...researchAttempt.report
    }];

    if (!researchAttempt.research.valid) {
      const fallbackWindow = getRollingResearchWindow(week);
      const fallbackRawResearch = await researchWeek(fallbackWindow, model, { fallback: true });
      researchAttempt = await validateAndVerifyResearch(fallbackRawResearch, fallbackWindow);
      researchAttempts.push({
        strategy: "recent_fallback",
        window: { start: fallbackWindow.start, end: fallbackWindow.end },
        ...researchAttempt.report
      });
    }

    const finalResearch = researchAttempt.research;
    const finalReport = researchAttempts.at(-1);
    await updateRun(admin, run.id, {
      status: finalResearch.valid ? "validated_research" : "rejected",
      candidate_count: finalReport.candidateCount,
      source_count: finalResearch.sources.length,
      topic_count: finalResearch.topics.length,
      research_snapshot: { sources: finalResearch.sources, topics: finalResearch.topics },
      validation_report: { reasons: finalResearch.reasons, researchAttempts },
      rejection_reason: finalResearch.reasons.join(" ") || null
    });
    ...
    await updateRun(admin, run.id, { status: assessment.valid ? "fact_checked" : "rejected", quality_score: assessment.score, validation_report: { researchAttempts, assessment, factCheck }, rejection_reason: assessment.reasons.join(" ") || null });
    if (!assessment.valid) {
      await updateRun(admin, run.id, { finished_at: new Date().toISOString() });

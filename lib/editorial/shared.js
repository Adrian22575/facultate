...
export const EDITORIAL_MODEL = process.env.OPENAI_EDITORIAL_MODEL?.trim() || "gpt-5.6";
...
export const editorialSourceSchema = z.object({
  ...
  importance: z.number().int().min(0).max(20),
  recency: z.number().int().min(0).max(20),
  credibility: z.number().int().min(0).max(20),
  usefulness: z.number().int().min(0).max(15),
...
export function validateResearch(research, week) {
  ...
  const sources = [...deduped.values()].filter((source) => source.credibility >= 12 && source.relevance >= 10);
  ...
  const topics = (research.candidateTopics || [])
    ...
    .filter((topic) => topic.sourceIds.length && topic.score >= 60)
  ...
  const hasPrimary = selected.some((topic) => topic.sourceIds.some((id) => sources.find((source) => source.id === id)?.sourceType === "primary" || sources.find((source) => source.id === id)?.primarySourceUrl));
  const reasons = [];

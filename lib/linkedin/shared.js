...
export const linkedInDraftSchema = z.object({
  ...
  articleUrl: z.string().url().max(1200),
  closingQuestion: z.string().trim().min(20).max(260),
  hashtags: z.array(z.string().trim().regex(/^#[\p{L}\p{N}_]+$/u).max(60)).min(3).max(5),
  ...
  tone: z.enum(["profesional-natural", "profesional-personal", "profesional-analitic"]),
...
export function buildLinkedInFullPost({ hook, body, articleUrl, closingQuestion, hashtags }) {
  return [hook, body, articleUrl, closingQuestion, hashtags.join(" ")]
    .map((part) => String(part || "").trim())
    .filter(Boolean)
    .join("\n\n");
}

export function validateLinkedInDraft(payload, { article, articleUrl }) {
  ...
  if (value.characterCount !== value.fullPost.length && value.characterCount !== fullPost.length) reasons.push("character_count_mismatch");
  if (!value.closingQuestion.endsWith("?") || genericQuestions.some((pattern) => pattern.test(value.closingQuestion))) reasons.push("generic_closing_question");

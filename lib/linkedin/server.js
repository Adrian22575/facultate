...
async function ensurePost(admin, articleId, connectionId) {
  ...
  const { data, error } = await admin.from("linkedin_editorial_posts").insert({ article_id: articleId, connection_id: connectionId, edition_number: 1, status: "not_generated" }).select(POST_SELECT).maybeSingle();
...
async function createNextEdition(admin, post) {
  const nextEdition = (post.edition_number || 1) + 1;
  const { data, error } = await admin.from("linkedin_editorial_posts").insert({ article_id: post.article_id, connection_id: post.connection_id, edition_number: nextEdition, status: "not_generated" }).select(POST_SELECT).maybeSingle();
  if (!error && data) return data;
  if (error?.code !== "23505") throw error || new Error("linkedin_post_edition_not_created");
  const { data: raced, error: racedError } = await admin.from("linkedin_editorial_posts").select(POST_SELECT).eq("article_id", post.article_id).eq("connection_id", post.connection_id).eq("edition_number", nextEdition).maybeSingle();
...
async function generateDraftPayload({ article, model, templateKey, objectiveKey, voiceKey }) {
  ...
  const assessment = validateLinkedInDraft(response.output_parsed, { article, articleUrl: url, objective: objectiveKey });
...
export async function prepareLinkedInDraft(articleId, { force = false, manual = false, templateKey, objectiveKey, voiceKey } = {}) {
  const admin = createAdminClient();

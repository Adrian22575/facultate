...
export async function prepareLinkedInDraft(articleId, { force = false, manual = false, createNewEdition = false, targetPostId, templateKey, objectiveKey, voiceKey, audienceKey, customAudience, ctaKey, narrativeKey, lengthKey, linkPlacementKey } = {}) {
  const admin = createAdminClient();
  ...
  let post;
  if (targetPostId) {
    const { data: targetPost, error: targetError } = await admin.from("linkedin_editorial_posts").select(POST_SELECT).eq("id", targetPostId).eq("article_id", article.id).maybeSingle();
    if (targetError) throw targetError;
    if (!targetPost) throw new Error("linkedin_post_not_found");
    if (targetPost.connection_id !== connection.id) return { ok: false, skipped: true, reason: "linkedin_connection_changed", post: targetPost };
    if (targetPost.status === "published") return { ok: true, skipped: true, reason: "already_published", post: targetPost };
    if (targetPost.status === "publishing") return { ok: true, skipped: true, reason: "already_publishing", post: targetPost };
    post = targetPost;
  } else {
    post = await ensurePost(admin, article.id, connection.id);
  }
  if (!targetPostId && createNewEdition && post.status !== "not_generated") {
    post = await createNextEdition(admin, post);
  } else if (!targetPostId && post.status === "published") {
  ...
  const savedOptions = targetPostId ? optionsFromPost(post) : {};
  const options = normalizeLinkedInGenerationOptions({
    templateKey: templateKey || savedOptions.templateKey || settings.default_template,
    objectiveKey: objectiveKey || savedOptions.objectiveKey || settings.default_objective,
    voiceKey: voiceKey || savedOptions.voiceKey || settings.default_voice,
    audienceKey: audienceKey || savedOptions.audienceKey || settings.default_audience,
    customAudience: customAudience || savedOptions.customAudience || settings.default_custom_audience,
    ctaKey: ctaKey || savedOptions.ctaKey || settings.default_cta,
    narrativeKey: narrativeKey || savedOptions.narrativeKey || settings.default_narrative,
    lengthKey: lengthKey || savedOptions.lengthKey || settings.default_length,
    linkPlacementKey: linkPlacementKey || savedOptions.linkPlacementKey || settings.default_link_placement
  });
  const optionColumns = {

...
export async function getLinkedInSettings(admin = createAdminClient()) {
  ...
  return data ? { ...data, model: normalizeLinkedInModel(data.model) } : DEFAULT_SETTINGS;
}
...
export async function getLinkedInAdminOverview() {
  ...
  try {
    ...
    return {
      config,
      settings: settings ? { ...settings, model: normalizeLinkedInModel(settings.model) } : DEFAULT_SETTINGS,
      connection: (connections || []).find((item) => item.status !== "disconnected") || connections?.[0] || null,
      posts: posts || [],
      generationPreview: getLinkedInGenerationPreview(normalizeLinkedInModel(settings?.model || DEFAULT_SETTINGS.model)),
      warning: null
    };
  } catch (error) {
...
export async function prepareLinkedInDraft(articleId, { force = false } = {}) {
  ...
  try {
    const draft = await generateDraftPayload({ article, model: normalizeLinkedInModel(settings.model || DEFAULT_SETTINGS.model) });
...
export async function saveLinkedInSettings(value, userId) {
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.from("linkedin_automation_settings").upsert({ singleton: true, mode: value.mode, notify_telegram: Boolean(value.notifyTelegram), include_article_image: false, fallback_to_text: true, model, updated_by: userId }, { onConflict: "singleton" }).select("singleton, mode, notify_telegram, include_article_image, fallback_to_text, model, updated_at").maybeSingle();

...
function canAccessPublishedBank(userId, membership, bank) {
  if (!bank) {
    return false;
  }
...
function mapQuestionBankItems(items) {
...
async function getLatestAccessiblePublishedQuestionBankMetaForSubject({ subjectId, userId, membership }) {
  ...
  const { data: candidateBanks, error: banksError } = await supabase
    ...
    .select(
      "id, user_id, subject_id, status, published_at, visibility_scope, target_cohort_id, target_unit_id, target_institution_id"
    )
    .eq("status", "published")
    .eq("exam_type", "normal")
...
async function getLatestPublishedQuestionBankForSubject({ subjectId, userId, membership }) {
  ...
  return {
    ...
    questions: mapQuestionBankItems(itemRows)
  };
}
...
async function getPublishedLicentaQuestionBanks({ userId, membership }) {
  ...
  const { data: candidateBanks, error: banksError } = await supabase
    .from("ai_question_banks")
    .select(
      "id, user_id, subject_id, subject_name, status, published_at, visibility_scope, target_cohort_id, target_unit_id, target_institution_id"
    )
    .eq("status", "published")
  ...
  const subjectMap = new Map(
    selectedBanks.map((bank) => [bank.id, { subjectId: bank.subject_id, subjectTitle: bank.subject_name }])
  );
  const { data: itemRows, error: itemsError } = await supabase
    .from("ai_question_bank_items")
...
export async function getSubjectProgressSnapshot(userId, subjectId) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("subject_progress")
      .select(
        "study_total_questions,study_viewed_question_ids,study_viewed_count,interactive_total_questions,interactive_answered,interactive_correct,interactive_wrong,test_best_score_percent,test_last_score_percent,last_mode,last_activity_at"
      )
      .eq("user_id", userId)
      .eq("subject_id", subjectId)
      .maybeSingle();

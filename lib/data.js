...
import { getSubjectProgressSummary } from "@/lib/subject-library";
...
export async function getLicentaExamAvailability({ userId, membership }) {
  if (!userId || !membership) return { questionCount: 0, subjectCount: 0 };

  const supabase = createAdminClient();
  const { data: candidateBanks, error: banksError } = await supabase
    .from("ai_question_banks")
    .select("id, subject_id, visibility_scope, target_cohort_id, target_unit_id, target_institution_id")
    .eq("status", "published")
    .eq("exam_type", "licenta")
    .limit(100);
  ...
  const selectedBySubject = new Map();
  for (const bank of candidateBanks || []) {
    if (!canAccessPublishedBank(userId, membership, bank)) continue;
    const key = bank.subject_id ? `subject:${bank.subject_id}` : `bank:${bank.id}`;
    if (!selectedBySubject.has(key)) selectedBySubject.set(key, bank);
  }

  const selectedBanks = Array.from(selectedBySubject.values());
  if (!selectedBanks.length) return { questionCount: 0, subjectCount: 0 };

  const { count, error: countError } = await supabase
    .from("ai_question_bank_items")
    .select("id", { count: "exact", head: true })
    .in("bank_id", selectedBanks.map((bank) => bank.id));

  if (countError) throw countError;
  return { questionCount: count || 0, subjectCount: selectedBanks.length };
}
...
function buildSubjectProgressDescription(row) {
  const mistakeCount = Array.isArray(row.mistake_question_ids)
    ...
    : 0;
  const progress = getSubjectProgressSummary(row);
  const studyPercent = progress.study.percent;
  const interactivePercent = progress.interactive.percent;
  const testPercent = progress.testPercent;
...
export async function getAccessibleSubjectsForUser({ userId, membership, userType = "student" }) {
  const library = await getSubjectLibraryForUser({ userId, membership, userType });

  return {
    subjects: library.subjects,
    subjectAllocations: library.subjectAllocations
  };
}

export async function getSubjectLibraryForUser({ userId, membership, userType = "student" }) {
  ...
  const latestBankBySubject = new Map();
  ...
  for (const bank of banks) {
    ...
    if (!latestBankBySubject.has(subject.id)) {
      latestBankBySubject.set(subject.id, bank);
    }
  ...
  const selectedBankIds = Array.from(latestBankBySubject.values()).map((bank) => bank.id);
  const accessibleSubjectIds = Array.from(accessibleSubjects.keys());
  const supabase = createAdminClient();
  const [{ data: questionRows, error: questionRowsError }, { data: progressRows, error: progressError }] =
    await Promise.all([
      selectedBankIds.length
        ? supabase.from("ai_question_bank_items").select("bank_id").in("bank_id", selectedBankIds)
        : Promise.resolve({ data: [], error: null }),
      accessibleSubjectIds.length
        ? supabase
            .from("subject_progress")
            .select(
              "subject_id, study_total_questions, study_viewed_count, interactive_total_questions, interactive_answered, test_best_score_percent, last_mode, last_activity_at"
            )
            .eq("user_id", userId)
            .in("subject_id", accessibleSubjectIds)
        : Promise.resolve({ data: [], error: null })
    ]);
  ...
  const questionCountByBank = new Map();
  for (const row of questionRows || []) {
    questionCountByBank.set(row.bank_id, (questionCountByBank.get(row.bank_id) || 0) + 1);
  }

  const progressBySubject = new Map((progressRows || []).map((row) => [row.subject_id, row]));
  const subjectLibrary = sortSubjects(Array.from(accessibleSubjects.values())).map((subject) => {
    const progress = getSubjectProgressSummary(progressBySubject.get(subject.id));
    const latestBank = latestBankBySubject.get(subject.id);

    return {
      ...subject,
      questionCount: latestBank ? questionCountByBank.get(latestBank.id) || 0 : 0,
      progress,
  ...
  return {
    subjects: subjectLibrary.map(({ questionCount, progress, lastActivityAt, lastMode, ...subject }) => subject),
...
async function createLearningSubjectSupabase({ title, createdByUserId }) {
  const trimmedTitle = title.trim();
  const normalizedTitle = normalizeText(trimmedTitle);
  const requestedSlug = createSubjectSlug(trimmedTitle);
  const supabase = createAdminClient();
  const { subjects } = await readSupabaseCatalog();

  const existingSubject = subjects.find(
    (item) => normalizeText(item.title) === normalizedTitle || item.id === requestedSlug
  );
  if (existingSubject) {
    return { subject: existingSubject, subjectCreated: false };
  }
  ...
  if (!insertError) {
    return { subject: mapSupabaseSubject(insertedSubject), subjectCreated: true };
  }
  ...
  const { data: concurrentSubject, error: selectError } = await supabase
    .from("subjects")
    .select("id, title, questions_file")
    .eq("id", subjectId)
    .single();
  ...
  return { subject: mapSupabaseSubject(concurrentSubject), subjectCreated: false };
}
...
export async function createLearningSubject({ title, createdByUserId }) {
  try {
    getSupabaseServerEnv();
    return await createLearningSubjectSupabase({ title, createdByUserId });
  } catch (error) {
    if (isSupabaseSetupIncompleteError(error)) {
      throw new Error(
        "Catalogul de materii din Supabase nu este gata inca. Ruleaza si verifica migrarile pana la `0008_subject_catalog.sql`."

...
export const freeTools = [
  {
    ...
    cta: { label: "Exersează grilele pe materii", href: "/auth/exit-demo?target=login&next=/materii" },
  ...
  },
  {
    ...
    cta: { label: "Transformă materialul într-un mod de studiu", href: "/auth/exit-demo?target=login&next=/materiale/invata" },
  ...
  },
  {
    ...
    cta: { label: "Creează teste din propria materie", href: "/auth/exit-demo?target=login&next=/materiale/invata" },
  ...
  },
  {
    ...
    cta: { label: "Începe o simulare completă", href: "/auth/exit-demo?target=login&next=/materii" },
  ...
  },
  {
    ...
    cta: { label: "Urmărește progresul în Nota 5+", href: "/auth/exit-demo?target=login&next=/materii" },
...
function reservePlanSlots(length, count, type, reserved) {
  for (let position = 1; position <= count; position += 1) {
    const preferred = Math.floor((position * (length + 1)) / (count + 1)) - 1;

    for (let distance = 0; distance < length; distance += 1) {
      const candidates = distance ? [preferred + distance, preferred - distance] : [preferred];
      const available = candidates.find((index) => index >= 0 && index < length && !reserved.has(index));

      if (available !== undefined) {
        reserved.set(available, type);
        break;
      }
    }
  }
...
export function generateStudyPlan(input = {}) {
  ...
  reservePlanSlots(possibleDays.length, simulationSlots, "simulation", reservedSlots);
  reservePlanSlots(possibleDays.length, reviewSlots, "review", reservedSlots);
  const plan = [];
  let volumeLeft = remaining;

  possibleDays.forEach((date, index) => {
    const type = reservedSlots.get(index) || "study";
...
export function calculateRequiredSimulationScore(input = {}) {
  const rawScores = Array.isArray(input.scores) ? input.scores.map((score) => String(score).trim()).filter(Boolean) : [];
  const scores = rawScores.map((score) => asNumber(score, NaN));
  ...
  if (scores.some((score) => !Number.isFinite(score) || score < 0 || score > 100)) {
    return { ok: false, error: "Fiecare scor trebuie să fie un număr între 0 și 100." };
  }

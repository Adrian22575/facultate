...
import {
  buildPersonalLicentaExamStatsDashboard,
...
export async function buildOverallStatsDashboard({ admin, academicContext, userId }) {
  const [
    ...
    personalLearningAttempts
  ] = await Promise.all([
    buildPersonalLicentaExamStatsDashboard({ admin, userId }),
    fetchSubjects(admin),
    ...
    fetchLearningAttempts(admin, [userId])
  ]);

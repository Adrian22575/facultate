...
function isOptionalLearningSchemaError(error) {
  return error?.code === "42P01" || error?.code === "42703";
}

async function fetchLearningAttempts(admin, userIds) {
  if (!userIds.length) return [];

  const { data, error } = await admin
    .from("learning_attempts")
    .select("id, study_set_id, user_id, mode, score_percent, correct_count, question_count, wrong_count, created_at")
    .in("user_id", userIds.slice(0, 1000))
    .order("created_at", { ascending: false })
    .limit(5000);

  if (error) {
    if (isOptionalLearningSchemaError(error)) return [];
    throw error;
  }
...
async function fetchPersonalLearningReviews(admin, userId) {
  const { data, error } = await admin
    .from("learning_flashcard_reviews")
    .select("study_set_id, rating, review_count, next_review_at, updated_at")
    .eq("user_id", userId)
...
async function fetchPersonalLearningStudySets(admin, userId) {
  const { data, error } = await admin
    .from("learning_study_sets")
    .select("id, user_id, title, status, chapter_count, concept_count, flashcard_count, question_count, published_at, updated_at")
    .eq("user_id", userId)
...
async function fetchLearningStudySetsByIds(admin, studySetIds) {
  if (!studySetIds.length) return [];

  const { data, error } = await admin
    .from("learning_study_sets")
    .select("id, user_id, title, status, chapter_count, concept_count, flashcard_count, question_count, published_at, updated_at")
    .in("id", studySetIds.slice(0, 1000));
...
function buildSubjectTrend(rows) {
  return rows
    ...
    .map((row, index) => ({
      ...
      label: row.lastActivityLabel || `Activitatea ${index + 1}`,
...
function buildLearningStats({
  ...
  const studySetMap = new Map(studySets.map((studySet) => [studySet.id, studySet]));
  const otherCommunityAttempts = communityAttempts.filter((attempt) => attempt.user_id !== currentUserId);
  const personalScores = personalAttempts
    .filter((attempt) => attempt.score_percent !== null && attempt.score_percent !== undefined)
    .map((attempt) => Number(attempt.score_percent))
    .filter((score) => Number.isFinite(score));
  const communityScores = otherCommunityAttempts
    .filter((attempt) => attempt.score_percent !== null && attempt.score_percent !== undefined)
    .map((attempt) => Number(attempt.score_percent))
    .filter((score) => Number.isFinite(score));
  const communityQuestionsByUser = new Map();
  for (const attempt of otherCommunityAttempts) {
    communityQuestionsByUser.set(
      attempt.user_id,
      (communityQuestionsByUser.get(attempt.user_id) || 0) + Number(attempt.question_count || 0)
    );
  }
  const reviewCount = reviews.reduce((total, review) => total + Number(review.review_count || 0), 0);
  const activeStudySetIds = new Set([
    ...personalAttempts.map((attempt) => attempt.study_set_id),
    ...reviews.map((review) => review.study_set_id)
  ].filter(Boolean));
  const usedStudySetIds = new Set([
    ...studySets.filter((studySet) => studySet.user_id === currentUserId).map((studySet) => studySet.id),
    ...activeStudySetIds
  ]);
  const attemptsByStudySet = new Map();
  const communityScoresByStudySet = new Map();
  const reviewsByStudySet = new Map();

  for (const attempt of personalAttempts) {
    const current = attemptsByStudySet.get(attempt.study_set_id) || [];
    current.push(attempt);
    attemptsByStudySet.set(attempt.study_set_id, current);
  }

  for (const attempt of otherCommunityAttempts) {
    if (attempt.score_percent === null || attempt.score_percent === undefined) continue;
    const score = Number(attempt.score_percent);
    if (!Number.isFinite(score)) continue;
    const current = communityScoresByStudySet.get(attempt.study_set_id) || [];
    current.push(score);
    communityScoresByStudySet.set(attempt.study_set_id, current);
  }

  for (const review of reviews) {
    const current = reviewsByStudySet.get(review.study_set_id) || [];
    current.push(review);
    reviewsByStudySet.set(review.study_set_id, current);
  }

  const rows = Array.from(usedStudySetIds)
    .map((studySetId) => {
      const studySet = studySetMap.get(studySetId);
      const attempts = attemptsByStudySet.get(studySetId) || [];
      const setReviews = reviewsByStudySet.get(studySetId) || [];
      const scores = attempts
        .filter((attempt) => attempt.score_percent !== null && attempt.score_percent !== undefined)
        .map((attempt) => Number(attempt.score_percent))
        .filter((score) => Number.isFinite(score));
      const latestAttemptAt = attempts[0]?.created_at || null;
      const latestReviewAt = setReviews[0]?.updated_at || null;
      const lastActivityAt = [latestAttemptAt, latestReviewAt, studySet?.updated_at]
        .filter(Boolean)
        .sort((left, right) => Date.parse(right) - Date.parse(left))[0] || null;

      return {
        key: studySetId,
        title: studySet?.title || "Material de invatare",
        attemptCount: attempts.length,
        questionCount: attempts.reduce((total, attempt) => total + Number(attempt.question_count || 0), 0),
        flashcardReviewCount: setReviews.reduce((total, review) => total + Number(review.review_count || 0), 0),
        bestScore: scores.length ? Math.max(...scores) : null,
        communityAverage: average(communityScoresByStudySet.get(studySetId) || []),
        lastActivityLabel: formatDateLabel(lastActivityAt),
        lastActivityAt
      };
    })
  ...
  const testAttemptCount = personalAttempts.filter((attempt) =>
    ["quick_test", "custom_test"].includes(attempt.mode)
  ).length;
  const mistakesAttemptCount = personalAttempts.filter((attempt) => attempt.mode === "mistakes").length;
  const modeCounts = [
    { key: "tests", label: "Teste", count: testAttemptCount },
    { key: "mistakes", label: "Greseli", count: mistakesAttemptCount },
    { key: "flashcards", label: "Flashcards", count: reviewCount }
  ];
  const modeTotal = Math.max(modeCounts.reduce((total, row) => total + row.count, 0), 1);

  return {
    scopeLabel: communityScope?.label || "comunitatea ta",
    overview: {
      studySetCount: usedStudySetIds.size,
      activeStudySetCount: activeStudySetIds.size,
      attemptCount: personalAttempts.length,
      answeredQuestions: personalAttempts.reduce(
        (total, attempt) => total + Number(attempt.question_count || 0),
        0
      ),
      personalAverageScore: average(personalScores),
      communityAverageScore: average(communityScores),
      communityAverageQuestions: average(Array.from(communityQuestionsByUser.values())),
      flashcardReviewCount: reviewCount,
      knownFlashcards: reviews.filter((review) => review.rating === "stiu").length,
      weakFlashcards: reviews.filter((review) => ["nu_stiu", "aproape"].includes(review.rating)).length,
      dueFlashcards: reviews.filter(
        (review) => review.next_review_at && Date.parse(review.next_review_at) <= Date.now()
      ).length,
      publishedStudySetCount: studySets.filter(
        (studySet) => studySet.user_id === currentUserId && studySet.published_at
      ).length,
      participantCount: new Set(communityAttempts.map((attempt) => attempt.user_id).filter(Boolean)).size
    },
    rows,
    modeMix: modeCounts.map((row) => ({
      ...row,
      percent: Math.round((row.count / modeTotal) * 100)
    })),
    trend: personalAttempts
      .filter(
        (attempt) =>
          attempt.score_percent !== null &&
          attempt.score_percent !== undefined &&
          Number.isFinite(Number(attempt.score_percent))
      )
      .slice(0, 10)
      .reverse()
      .map((attempt, index) => ({
        key: attempt.id || `${attempt.study_set_id}-${index}`,
        label: formatDateLabel(attempt.created_at) || `T${index + 1}`,
        score: Number(attempt.score_percent),
        mode: studySetMap.get(attempt.study_set_id)?.title || "Material"
      }))
  };
}
...
function buildCompetitionStats({
  userIds,
  currentUserId,
  profileMap,
  subjectRows,
  ...
  for (const attempt of learningAttempts) {
    ...
    user.effort += effort;
    if (score !== null && Number.isFinite(score)) user.scoreParts.push(score);

    if (isAfterDate(attempt.created_at, weekStart)) {
      user.weekEffort += effort;
      if (score !== null && Number.isFinite(score)) user.weekScoreParts.push(score);
    }

    if (isAfterDate(attempt.created_at, monthStart)) {
      user.monthEffort += effort;
      if (score !== null && Number.isFinite(score)) user.monthScoreParts.push(score);
    }
  }
...
export async function buildOverallStatsDashboard({ admin, academicContext, userId }) {
  const [
    ...
    personalSubjectRows,
    subjectScopeData,
    personalLearningStudySets,
    personalLearningReviews
  ] = await Promise.all([
    ...
    fetchPersonalLearningStudySets(admin, userId),
    fetchPersonalLearningReviews(admin, userId)
  ]);
  const communityUserIds = Array.from(new Set([...subjectScopeData.userIds, userId]));
  const [
    communitySubjectRows,
    profileMap,
    communityLicentaAttempts,
    communityLearningAttempts
  ] = await Promise.all([
    ...
    fetchLearningAttempts(admin, communityUserIds)
  ]);
  const personalLearningAttempts = communityLearningAttempts.filter((attempt) => attempt.user_id === userId);
  const learningStudySetIds = Array.from(
    new Set([
      ...personalLearningStudySets.map((studySet) => studySet.id),
      ...personalLearningAttempts.map((attempt) => attempt.study_set_id),
      ...personalLearningReviews.map((review) => review.study_set_id)
    ].filter(Boolean))
  );
  const relatedLearningStudySets = await fetchLearningStudySetsByIds(admin, learningStudySetIds);
  const learningStudySets = Array.from(
    new Map(
      [...personalLearningStudySets, ...relatedLearningStudySets].map((studySet) => [studySet.id, studySet])
    ).values()
  );
  ...
  const learning = buildLearningStats({
    studySets: learningStudySets,
    personalAttempts: personalLearningAttempts,
    communityAttempts: communityLearningAttempts,
    reviews: personalLearningReviews,
  ...
  const competition = buildCompetitionStats({
    ...
    learningAttempts: communityLearningAttempts
  });
  ...
  const overall = {
    totalActions:
      ...
      learning.overview.answeredQuestions +
      learning.overview.flashcardReviewCount,
    activeAreas: [
      licenta.overview.personalAttemptCount > 0,
      subjects.overview.activeSubjectCount > 0,
      learning.overview.activeStudySetCount > 0
    ...
    averageScore: average(
      [
        ...
        learning.overview.personalAverageScore
      ].filter((score) => score > 0)
    ),
    communityAverageScore: average(
      [
        ...
        learning.overview.communityAverageScore
      ].filter((score) => score > 0)
    )
  };

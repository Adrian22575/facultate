...
export async function getAdminFeedbackEntries() {
  ...
  const { data, error } = await admin
    ...
    .select("id, user_email, user_type, feedback_type, message, optional_detail, page_path, screenshot_path, created_at")
...
async function getAdminHeaderTestimonialRows(admin) {
  ...
  if (result.error) {
    if (isSupabaseSetupIncompleteError(result.error)) {
      return [];
    }
    throw result.error;
  }
...
async function getAdminHeaderProcessingRows(admin) {
  ...
  if (logsResult.error) {
    return { rows: [], jobQueryCount: 0, loadedRows: [] };
  }

  const logRows = logsResult.data || [];
  const jobIds = Array.from(new Set(logRows.map((row) => row.job_id).filter(Boolean)));

  if (!jobIds.length) {
    return { rows: logRows, jobQueryCount: 0, loadedRows: logRows };
  }
  ...
  if (jobsResult.error) {
    return { rows: [], jobQueryCount: 1, loadedRows: logRows };
  }

  const jobs = jobsResult.data || [];
  const jobStatusMap = new Map(jobs.map((row) => [row.id, row.status || null]));

  return {
    rows: logRows.map((row) => ({
      ...row,
      job_status: row.job_id ? jobStatusMap.get(row.job_id) || null : null
    })),
    jobQueryCount: 1,
    loadedRows: [...logRows, ...jobs]
  };
}

async function loadAdminHeaderActionSummary(adminUserId = null) {
  ...
  const [
    ...
    admin
      ...
      .limit(30),
    getAdminHeaderTestimonialRows(admin),
    admin
      .from("ai_generation_jobs")
      .select("id, source_document_id, created_at, completed_at")
    ...
    admin
      ...
      .order("created_at", { ascending: false })
      .limit(200),
    getAdminHeaderProcessingRows(admin),
    getAdminNotificationViews(adminUserId)
  ]);
  ...
  const failedUploads = buildAdminHeaderFailedUploadRows(failedJobs, failedSources);
  const summary = buildAdminActionSummary({
    testimonialRewardEntries,
    failedUploads,
    openAILogs: processingResult.rows,
    billingData: { webhookRows },
    notificationViews
  });
  const loadedPayload = {
    ...
    failedJobs,
    failedSources,
    processingRows: processingResult.loadedRows,
    notificationViews
  };
  ...
  recordServerTiming(
    ...
    {
      loaded_bytes: new TextEncoder().encode(JSON.stringify(loadedPayload)).byteLength,
      result_bytes: new TextEncoder().encode(JSON.stringify(summary)).byteLength,
      round_trips: 6 + processingResult.jobQueryCount
    }
  );
...
export async function getAdminHeaderActionSummary(adminUserId = null) {
  return loadAdminHeaderActionSummary(adminUserId);
}

...
export async function notifyEditorialDraftReady({ article, week, runKey }) {
  return notifyTelegramAdmin({
    ...
    lines: [
      ...
      { label: "Admin", value: absoluteInternalLink("/admin/continut/articole") }
    ],
...
export async function notifyLinkedInDraftReady({ post, article, mode }) {
  return notifyTelegramAdmin({
    ...
    lines: [
      ...
      { label: "Aprobare", value: absoluteInternalLink(`/admin/continut/articole/${article.id}?tab=linkedin&linkedin_post=${post.id}`) }
    ],
...
export async function notifyLinkedInFailed({ postId, article, stage, reason, published = false, reconnect = false, occurrence = new Date().toISOString() }) {
  return notifyTelegramAdmin({
    // Every manual retry is a distinct operational event. Deduplicating only
    // by its reason hid later failures and made recovery impossible to audit.
    eventKey: `linkedin_failed:${postId}:${stage}:${occurrence}`,
    ...
    lines: [
      ...
      { label: "Admin", value: absoluteInternalLink(`/admin/continut/articole/${article.id}?tab=linkedin&linkedin_post=${postId}`) }
    ],
...
export async function notifyAdminTestimonialReviewSubmitted({ submission, user }) {
  ...
  const adminPath = absoluteInternalLink("/admin/comunitate/testimoniale");
  const rewardLabel =

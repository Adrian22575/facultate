...
export async function getPrivateGeneratedTests(
  userId,
  { academicContext, academicContextPromise } = {}
) {
  const supabase = createAdminClient();
  const contextPromise =
    academicContextPromise ||
    (academicContext ? Promise.resolve(academicContext) : getAcademicContext(userId));
  const ownTestsPromise = supabase
    .from("user_generated_tests")
    .select(
  ...
  const [context, { data, error }] = await Promise.all([
    contextPromise,
    ownTestsPromise
  ]);

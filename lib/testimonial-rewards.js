...
async function getPremiumRewardWindowStart(admin, userId) {
  const { data, error } = await admin
...
async function grantTestimonialReward(admin, submission) {
  ...
  if (submission.reward_type === "premium_24h") {
    const startsAt = await getPremiumRewardWindowStart(admin, submission.user_id);
    ...
    if (grant?.id) {
      return { premiumGrantId: grant.id, creditLedgerId: null };
    }
    ...
    return { premiumGrantId: existingGrant?.id || null, creditLedgerId: null };
  }

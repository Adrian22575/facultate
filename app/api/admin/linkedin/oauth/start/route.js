...
export async function GET(request) {
  ...
  if (!getLinkedInConfigStatus().ready) return NextResponse.redirect(new URL("/admin?admin_tab=editorial&linkedin_error=not_configured#linkedin-distribution", request.url));
  ...
  try {
    ...
    const { error } = await admin.from("linkedin_oauth_states").insert({
      ...
      return_path: "/admin?admin_tab=editorial#linkedin-distribution",
    ...
    return NextResponse.redirect(new URL(`/admin?admin_tab=editorial&linkedin_error=${code}#linkedin-distribution`, request.url));
  }
}

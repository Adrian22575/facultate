...
export async function GET(request) {
  ...
  if (!user || !(await isAdminUser(user))) return NextResponse.redirect(new URL("/auth/login?next=/admin/continut/linkedin", request.url));
  if (!getLinkedInConfigStatus().ready) return NextResponse.redirect(new URL("/admin/continut/linkedin?linkedin_error=not_configured#editorial-workspace", request.url));
  ...
  try {
    ...
    const { error } = await admin.from("linkedin_oauth_states").insert({
      ...
      return_path: "/admin/continut/linkedin#editorial-workspace",
    ...
    return NextResponse.redirect(new URL(`/admin/continut/linkedin?linkedin_error=${code}#editorial-workspace`, request.url));
  }
}
